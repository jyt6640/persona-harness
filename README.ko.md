...
> 안정 npm 채널은 `persona-harness@latest=0.5.0`입니다. release prep 대상은 prerelease `next` 채널용 `0.6.0-rc.1`입니다. QA publish GO와 registry gitHead/shasum 검증 전까지 registry `next`는 `0.5.0-rc.2`를 유지하며, alpha는 `0.3.9-alpha.8`입니다.
