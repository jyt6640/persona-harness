...
> 안정 npm 채널은 `persona-harness@latest=0.5.0`입니다. 게시된 prerelease 채널은 `next=0.6.0-rc.3`이고, QA publish-readiness 전 release prep target은 `0.6.0-rc.4`이며, alpha는 `0.3.9-alpha.8`입니다. 안정 `0.6.0`은 계속 보류 상태이고, `0.6.0-rc.4` prep은 향후 `next` publish와 External registry smoke 전까지 registry evidence가 아닙니다.
