...
> 안정 npm 채널은 `persona-harness@latest=0.5.0`입니다. prerelease 채널은 QA 승인 후 `0.6.0-rc.2`가 publish되기 전까지 현재 `next=0.6.0-rc.1`이고, alpha는 `0.3.9-alpha.8`입니다. `0.6.0-rc.2` prep은 `next` release-candidate target일 뿐, `latest` 이동이나 더 넓은 product-efficacy 주장이 아닙니다.
