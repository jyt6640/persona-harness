...
> 안정 npm 채널은 `persona-harness@latest=0.5.0`입니다. 게시된 prerelease 채널은 여전히 `next=0.6.0-rc.2`이고, release prep target은 별도 publish 전까지 `0.6.0-rc.3`이며, alpha는 `0.3.9-alpha.8`입니다. `0.6.0-rc.2` registry smoke는 release-candidate package-runtime evidence일 뿐, `latest` 이동이나 더 넓은 product-efficacy 주장이 아닙니다.
