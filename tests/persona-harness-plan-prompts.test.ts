...
describe("ph plan prompt and usage output", () => {
  it("prints an implementation prompt after the workflow plan is accepted", () => {
    ...
    expect(implement.stdout).toContain("PH-owned surfaces")
    expect(implement.stdout).toContain("Optional external codegraph/code-nav tools")
    expect(implement.stdout).toContain("do not present them as PH-owned or token-saving")
  ...
  it("prints a reusable plan-only OpenCode prompt", () => {
    ...
    expect(prompt.stdout).toContain("PH-owned surfaces")
    expect(prompt.stdout).toContain("외부 codegraph/code-nav 도구는 실제로 설치되어 있을 때만 선택적으로 사용")
