...
describe("release workflow policy", () => {
  it("requires the staging beta publish route to bind the immutable version tag and sanitized registry readback", () => {
    ...
    expect(readback).toContain("registry_readback_timeout_seconds=1200")
    expect(readback).toContain('registry_readback_started_at="$(date +%s)"')
    expect(readback).toContain("while :; do")
    expect(readback).toContain("registry_readback_elapsed_seconds=$(( $(date +%s) - registry_readback_started_at ))")
    expect(readback).toContain("if (( registry_readback_elapsed_seconds >= registry_readback_timeout_seconds )); then")
    expect(readback).toContain("registry_readback_remaining_seconds=$(( registry_readback_timeout_seconds - registry_readback_elapsed_seconds ))")
    expect(readback).toContain("registry_readback_sleep_seconds=$(( registry_readback_remaining_seconds < 10 ? registry_readback_remaining_seconds : 10 ))")
    expect(readback).toContain('sleep "$registry_readback_sleep_seconds"')
    expect(readback).not.toContain("for attempt in {1..30}; do")
  ...
  it("keeps the current source candidate unpublishable until a matching approval is recorded", () => {
    ...
    expect(acceptance.package).toMatchObject({ channel: "unpublished", scope: "source-candidate" })
    expect(checkDistTagCompatibility({
      approvalScope: acceptance.package.scope,
      distTag: "latest",
      version: packageVersion,
    })).toMatchObject({ code: "dist-tag-latest-approval", ok: false })
    expect(checkDistTagCompatibility({
      approvalScope: acceptance.package.scope,
      distTag: "staging",
      version: packageVersion,
    })).toMatchObject({ code: "dist-tag-staging-approval", ok: false })
    expect(checkDistTagCompatibility({
      approvalScope: acceptance.package.scope,
      distTag: "next",
      version: packageVersion,
