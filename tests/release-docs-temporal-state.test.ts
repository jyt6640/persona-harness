...
const publicReleaseDocs = [
  ...
  "docs/current/release/v0.8.0-beta.24-release-notes.md",
]

describe("release docs temporal-state boundary", () => {
  it("keeps historical final-observer evidence distinct from the beta.24 v4 procedure-only candidate", () => {
    ...
    expect(text).toContain("0.8.0-beta.24")
    ...
    expect(text).toContain("consumer-authority-beta24-acceptance.json")
    ...
    expect(text).toContain("stage-scoped NUL-safe")
  })
})
