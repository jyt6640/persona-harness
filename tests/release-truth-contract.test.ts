...
describe("public release truth", () => {
  it("keeps live release lookup out of package-visible current documents", () => {
    const releaseNotes = readRepositoryFile("docs/current/release/v0.8.30-release-notes.md")
    ...
    expect(changelog).toMatch(/^## \[0\.8\.29\] - 2026-08-24$/m)
    expect(packageIndex).toContain("| `0.8.29` | 2026-08-24 | published stable release")
