...
describe("public release truth", () => {
  it("keeps live release lookup out of package-visible current documents", () => {
    const releaseNotes = readRepositoryFile("docs/current/release/v0.8.27-release-notes.md")
    ...
    expect(changelog).toMatch(/^## \[0\.8\.26\] - 2026-08-22$/m)
    expect(packageIndex).toContain("| `0.8.26` | 2026-08-22 | published stable release")
