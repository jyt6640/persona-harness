...
describe("ph plan --next", () => {
  ...
  it("recommends continuation before review when the filled implementation report has remaining scope", () => {
    ...
    expect(result.stdout).toContain("Continuation evidence status: present")
    expect(result.stdout).toContain("Artifact: .persona/workflow/implementation-report.md")
    expect(result.stdout).not.toContain("README 221-end")
    expect(result.stdout).not.toContain("Step 3~7")
  })
...
describe("ph plan --resume", () => {
  ...
  it("prints a bounded continuation artifact reference without report prose", () => {
    const projectDir = createProfiledTempProject()
    expect(runPlan(projectDir, []).status).toBe(0)
    expect(runPlan(projectDir, ["--accept"]).status).toBe(0)
    const report = readFileSync(implementationReportPath(projectDir), "utf8")
    ...
    expect(result.stdout).toContain("Artifact: .persona/workflow/implementation-report.md")
    expect(result.stdout).toContain("Plan unchecked status: present")
    expect(result.stdout).toContain("Artifact: .persona/workflow/plan.md")
    expect(result.stdout).not.toContain("README ranges read: 1-200")
    expect(result.stdout).not.toContain("Step 1 basic CRUD")
    expect(result.stdout).not.toContain("Spring Boot source generation")
    expect(result.stdout).not.toContain("TUI output limit")
  })
})
...
describe("ph workflow continue", () => {
  it("aliases the accepted-plan continuation prompt for short TUI resume requests", () => {
    ...
    expect(result.stdout).toContain("Persona Harness resume prompt")
    expect(result.stdout).toContain("Artifact: .persona/workflow/implementation-report.md")
    expect(result.stdout).not.toContain("README Step 4/6/7")
    expect(result.stdout).not.toContain("import/export and sharing API")
    expect(result.stdout).toContain("npx ph workflow implement")
  })
  ...
  it("shows the pending ticket path and archive command before the generic resume prompt", () => {
    ...
    expect(result.stdout).toContain("Artifact: .persona/workflow/work/step-2/00-task-card.md")
    expect(result.stdout).not.toContain("Title: iCal Import/Export")
    expect(result.stdout).toContain("Next command: npx ph workflow next")
    expect(result.stdout).toContain("After review: npx ph workflow archive step-2")
  })
  ...
  it("shows review-report follow-up before archive guidance when a req ticket remains pending", () => {
    ...
    expect(result.stdout).toContain("Implementation report status: filled")
    expect(result.stdout).toContain("Review report status: template")
    expect(result.stdout).toContain("Step: fill-review-report")
    expect(result.stdout).toContain("Blocker: review-report-missing")
    expect(result.stdout).toContain("Status: pending")
    expect(result.stdout).toContain("Artifact: .persona/workflow/review-report.md")
    expect(result.stdout).toContain("Ticket: req-1")
    expect(result.stdout).toContain("After review: npx ph workflow archive req-1")
    expect(result.stdout).toContain("Archive is a candidate action only; do not auto-archive.")
    expect(result.stdout.indexOf("Step: fill-review-report")).toBeLessThan(result.stdout.indexOf("Step: archive-current-ticket"))
  })
  ...
  it("prioritizes failed verification repair in the continuation prompt", () => {
    ...
    expect(result.status).toBe(0)
    expect(result.stdout).toContain("Step: fix-verification")
    expect(result.stdout).toContain("Blocker: verification-failed")
    expect(result.stdout).toContain("Status: blocked")
    expect(result.stdout).toContain("Artifact: .persona/evidence")
    expect(result.stdout).toContain("Next command: after completing the action, run npx ph workflow check")
    expect(result.stdout).not.toContain("./gradlew test failed")
    expect(result.stdout).not.toContain("cannot find symbol")
  })
})
