...
describe.sequential("real Java/Spring Gradle cooperative Finish", () => {
  ...
  it("drives a fresh Java/Spring Gradle project through public bootstrap, reports, and cooperative Finish", () => {
    ...
    expect(defaultFinish.stderr).not.toContain("implementation-report-missing")
    expect(defaultFinish.stderr).not.toContain("review-report-missing")
    ...
    expect(defaultFinish.stderr).not.toContain("workflow-loop-state-absent")
    expect(defaultFinish.stderr).not.toContain("ralph-loop-state-absent")
    expect(existsSync(join(consumer, ".persona", "custom-evidence", "verification-receipts"))).toBe(false)
    expect(existsSync(join(consumer, ".persona", "custom-evidence", "verification-attempts"))).toBe(false)
    expect(existsSync(join(consumer, ".persona", "custom-evidence", "finish-attestation"))).toBe(false)
    expect(matchesAt(consumer, expected)).toBe(true)
    expect(cooperativeFinish.status, cooperativeFinish.stderr).toBe(0)
    expect(cooperativeFinish.stdout).toContain("Finish status: PASS")
    expect(matchesAt(consumer, expected)).toBe(true)
