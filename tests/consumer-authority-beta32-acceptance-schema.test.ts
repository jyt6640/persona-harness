...
describe("consumer authority beta.32 acceptance schema", () => {
  it("ships the known-completion policy with current package and root-bound contract authority", () => {
    const historical = JSON.parse(readFileSync(join(repositoryRoot, "docs", "current", "release", "consumer-authority-beta32-acceptance.json"), "utf8"))
    const manifest = parseBeta32AcceptanceManifest(historical, "0.8.0-beta.32")
  ...
  it("keeps beta32 strict and historical after current package preflights advance", () => {
    for (const script of [
      "preflight-consumer-authority-external-attestation.mjs",
      "preflight-consumer-authority-external-artifact-transport.mjs",
    ]) {
      const source = readFileSync(join(repositoryRoot, "scripts", script), "utf8")
      expect(source).not.toContain('from "./consumer-authority-beta32-acceptance-schema.mjs"')
      expect(source).not.toContain("readBeta32AcceptanceManifest(packageRoot)")
