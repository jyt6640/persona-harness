...
describe("historical consumer authority 0.8.4 acceptance schema", () => {
  ...
  it("rejects neighboring versions and records that attempt to reuse v082 as current", () => {
    const fixture = canonicalV084AcceptanceManifest()
    const packageRecord = fixture.package as Record<string, unknown>
    packageRecord.version = "0.8.2"

    expect(() => parseV084AcceptanceManifest(fixture, "0.8.4")).toThrow(V084AcceptanceManifestError)
    expect(() => parseV084AcceptanceManifest(canonicalV084AcceptanceManifest(), "0.8.2")).toThrow(V084AcceptanceManifestError)
    expect(() => parseV082AcceptanceManifest(canonicalV084AcceptanceManifest(), "0.8.4")).toThrow()
  })

  it("explicitly accepts a retained draft plan before V4 readiness can reach authority-only blocking", () => {
    const root = mkdtempSync(join(tmpdir(), "persona-v084-retained-plan-"))
    try {
  ...
  it("keeps both direct current preflights on the successor schema without changing their bounded behavior", () => {
    ...
    try {
      ...
      for (const script of [
        ...
        expect(source).toContain('from "./consumer-authority-v087-acceptance-schema.mjs"')
        expect(source).toContain("readV087AcceptanceManifest(packageRoot)")
