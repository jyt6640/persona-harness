...
describe("clean package boundary", () => {
  ...
  it("uses the shared portable consumer contract for authoritative bundle exercise mode", () => {
    ...
    expect(verifier).toContain("assessPackageExerciseContractOutput")
    expect(verifier).toContain("${fallbackCode}-${outcome.phase}-${outcome.code}")
    expect(verifier).toContain("${fallbackCode}-phase-envelope-invalid")
