...
describe("consumer authority 0.8.6 acceptance schema", () => {
  it("binds the current package and lock to a new strict 0.8.6 record while preserving v082 history", () => {
    const manifest = readV086AcceptanceManifest(repositoryRoot)
  ...
  it("rejects neighboring versions and records that attempt to reuse v082 as current", () => {
    ...
    expect(() => parseV086AcceptanceManifest(canonicalV086AcceptanceManifest(), "0.8.2")).toThrow(V086AcceptanceManifestError)
  ...
  it("explicitly accepts a retained draft plan before V4 readiness can reach authority-only blocking", () => {
    const root = mkdtempSync(join(tmpdir(), "persona-v086-retained-plan-"))
    try {
  ...
  it("routes both direct current preflights through v086 without changing their bounded behavior", () => {
    const root = mkdtempSync(join(tmpdir(), "persona-v086-preflight-"))
    ...
    try {
      ...
      for (const script of [
        ...
        expect(source).toContain('from "./consumer-authority-v086-acceptance-schema.mjs"')
        expect(source).toContain("readV086AcceptanceManifest(packageRoot)")
