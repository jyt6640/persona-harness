...
describe("Context program status", () => {
  it("distinguishes the released P0 boundary from the remaining live-host and product-value evidence", () => {
    ...
    const remainingWork = status.split("\n").find((line) => line.startsWith("Remaining bounded work:")) ?? ""
    ...
    expect(status).toContain("persona-harness@0.8.39")
    expect(status).toContain("ec3680bcceccf582521952dc77bf3cc9fb7cd874")
    expect(status).toContain("#442")
    expect(status).toContain("#443")
    expect(status).toContain("#446")
    expect(status).toContain("#449")
    expect(status).toContain("#450")
    expect(status).toContain("#451")
    expect(status).toContain("#452")
    expect(status).toContain("881be07b3e3baa443a66126ca94008499082b732")
    ...
    expect(remainingWork).not.toContain("metadata-only structural observer boundary in [#449]")
    expect(status).not.toContain("PRODUCT_GO")
