...
describe("ph plan", () => {
  it("creates a blackbear planning artifact from README and backend profile summary", () => {
    ...
    expect(implementationReport).toContain("`:bootJar SKIPPED` is not recorded as a passing build")
    ...
    expect(implementationReport).toContain("If manual QA was not possible, record the reason plus stderr and the key log lines.")
    expect(implementationReport).toContain("After filling this in, run `npx ph plan --report-filled implementation`.")
    expect(implementationReport).toContain("## Continuation")
    expect(implementationReport).toContain("README/plan ranges still unread:")
    expect(implementationReport).toContain("Requirements completed:")
    expect(implementationReport).toContain("Requirements not completed:")
    expect(implementationReport).toContain("Reason for stopping:")
    expect(implementationReport).toContain("Next command or task to resume with:")
    expect(implementationReport).toContain("Hint for the next prompt:")
    ...
    expect(reviewReport).toContain("The implementation report records the README/plan read method and ranges.")
    expect(reviewReport).toContain("## Boundary Review")
    expect(reviewReport).toContain("Reviewed the `npx ph bearshell gradle test` result.")
    expect(reviewReport).toContain("Reviewed the `npx ph bearshell gradle build` result.")
    expect(reviewReport).toContain("`:bootJar SKIPPED`")
    expect(reviewReport).toContain("`npx ph bearshell --shell 'gradle bootRun")
    expect(reviewReport).toContain("manual QA evidence for the HTTP happy path and failure path")
    expect(reviewReport).toContain("After filling this in, run `npx ph plan --report-filled review`.")
