...
describe("current package authority", () => {
  it("binds the root, lockfile, and private shared package to 0.8.9", () => {
    ...
    expect(sharedPackage.version).toBe("0.8.9")
  })
  ...
  it("ships a distinct current acceptance record while retaining historical records", () => {
    ...
    const currentSchema = "scripts/consumer-authority-v089-acceptance-schema.mjs"
    const currentTypes = "scripts/consumer-authority-v089-acceptance-schema.d.mts"
    const currentRecord = "docs/current/release/consumer-authority-v089-acceptance.json"
