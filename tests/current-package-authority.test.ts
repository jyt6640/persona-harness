...
describe("current package authority", () => {
  it("binds the root, lockfile, and private shared package to one current version", () => {
    ...
    expect(sharedPackage.version).toBe(packageJson.version)
  })

  it("ships a generic current acceptance record while retaining historical records", () => {
    const packageJson = JSON.parse(readFileSync(join(repositoryRoot, "package.json"), "utf8")) as {
      readonly files: readonly string[]
    }
    const currentSchema = "scripts/consumer-authority-current-acceptance-schema.mjs"
    const currentTypes = "scripts/consumer-authority-current-acceptance-schema.d.mts"
    const currentRecord = "docs/current/release/consumer-authority-current-acceptance.json"
    ...
    expect(existsSync(join(repositoryRoot, "scripts/consumer-authority-v0827-acceptance-schema.mjs"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "scripts/consumer-authority-v0829-acceptance-schema.mjs"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "scripts/consumer-authority-v0830-acceptance-schema.mjs"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "scripts/consumer-authority-v0831-acceptance-schema.mjs"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "scripts/consumer-authority-v0832-acceptance-schema.mjs"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "docs/current/release/consumer-authority-v0828-acceptance.json"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "docs/current/release/consumer-authority-v0829-acceptance.json"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "docs/current/release/consumer-authority-v0827-acceptance.json"))).toBe(true)
    ...
    expect(existsSync(join(repositoryRoot, "scripts/consumer-authority-v0824-acceptance-schema.mjs"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "scripts/consumer-authority-v0825-acceptance-schema.mjs"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "scripts/consumer-authority-v0826-acceptance-schema.mjs"))).toBe(true)
    ...
    expect(existsSync(join(repositoryRoot, "docs/current/release/consumer-authority-v0824-acceptance.json"))).toBe(true)
    expect(existsSync(join(repositoryRoot, "docs/current/release/consumer-authority-v0825-acceptance.json"))).toBe(true)
