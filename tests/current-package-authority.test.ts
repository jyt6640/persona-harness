...
describe("current package authority", () => {
  it("binds the root, lockfile, and private shared package to 0.8.5", () => {
    ...
    expect(packageLock.version).toBe("0.8.5")
    expect(packageLock.packages[""]?.version).toBe("0.8.5")
    expect(sharedPackage.version).toBe("0.8.5")
  })

  it("ships a distinct current acceptance record while retaining the 0.8.4 historical record", () => {
    const packageJson = JSON.parse(readFileSync(join(repositoryRoot, "package.json"), "utf8")) as {
      readonly files: readonly string[]
    }
    const currentSchema = "scripts/consumer-authority-v085-acceptance-schema.mjs"
    const currentTypes = "scripts/consumer-authority-v085-acceptance-schema.d.mts"
    const currentRecord = "docs/current/release/consumer-authority-v085-acceptance.json"
