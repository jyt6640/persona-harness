...
`,
    ...
    expect(output.promptPreview.join("\n")).toContain("Rule role: test-writer")
    expect(output.promptPreview.join("\n")).toContain("Rule stage: verification")
    expect(output.rulePackHash).toMatch(/^sha256:[a-f0-9]{64}$/u)
    expect(output.promptPreview.join("\n")).toContain(`Rule pack hash: ${output.rulePackHash}`)
    expect(output.promptPreview.join("\n")).not.toContain("verification scoped policy")
  ...
  it("runs capped fresh-session iterations and records prompt/log state", () => {
    ...
    expect(firstPrompt).toContain("Rule role: test-writer")
    expect(firstPrompt).toContain("Rule stage: verification")
    expect(firstPrompt).toMatch(/Rule pack hash: sha256:[a-f0-9]{64}/u)
    expect(firstPrompt).not.toContain("Scoped PH rules")
