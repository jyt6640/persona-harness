...
describe("ph workflow start and finish", () => {
  ...
  it("blocks finish with review-report guidance while preserving pending req ticket guidance", () => {
    ...
    expect(result.stderr).toContain("- closure blockers: 6")
    expect(result.stderr).toContain("- first blocker: verification-unknown")
    expect(result.stderr).toContain("- first next action: Run test/build/runtime verification through `npx ph bearshell`.")
    expect(result.stderr).toContain("- full blocker details follow for diagnostics")
