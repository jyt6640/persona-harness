...
describe("npm contributor test contract", () => {
  it("separates fast, unit, integration, smoke, full, and package evidence", () => {
    ...
    expect(scripts["benchmark:context"]).toBe("npm run build && node scripts/eval/run-context-comparison.mjs --manifest docs/current/context-comparison-manifest.json --current-checkout")
    expect(selectedTestFiles(scripts["test:unit"])).toContain("tests/bootstrap-write-boundary-security.test.ts")
    ...
    expect(selectedTestFiles(scripts["test:unit"])).toContain("tests/context-value-window.test.ts")
    ...
    expect(selectedTestFiles(scripts["test:unit"])).toContain("tests/context-program-status.test.ts")
    expect(selectedTestFiles(scripts["test:unit"])).toContain("tests/context-readme-boundary.test.ts")
    expect(selectedTestFiles(scripts["test:unit"])).toContain("tests/host-capability-manifest.test.ts")
    expect(selectedTestFiles(scripts["test:unit"])).toContain("tests/portable-skill-contract.test.ts")
    expect(selectedTestFiles(scripts["test:unit"])).toContain("tests/socratic-interview-core.test.ts")
    expect(selectedTestFiles(scripts["test:integration"])).toContain("tests/opencode-context-delivery.test.ts")
    expect(selectedTestFiles(scripts["test:unit"])).toContain("tests/context-rendered-budget.test.ts")
    expect(selectedTestFiles(scripts["test:unit"])).toContain("tests/context-delivery-store.test.ts")
    expect(selectedTestFiles(scripts["test:integration"])).toContain("tests/portable-context-package.test.ts")
    expect(selectedTestFiles(scripts["test:integration"])).toContain("tests/opencode-context-batch.test.ts")
    expect(selectedTestFiles(scripts["test:integration"])).toContain("tests/phase0-product-interview-hook.test.ts")
    expect(selectedTestFiles(scripts["test:integration"])).toContain("tests/product-deep-interview.test.ts")
    expect(selectedTestFiles(scripts["test:integration"])).toContain("tests/socratic-interview-command.test.ts")
