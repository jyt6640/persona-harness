...
describe("persona-harness init", () => {
  it("installs the Persona template and OpenCode plugin config without copying evidence", () => {
    ...
    const agentSkill = readFileSync(join(projectDir, ".agents", "skills", "persona-harness-deep-interview", "SKILL.md"), "utf8")
    expect(agentSkill).toContain("name: ph-deep-interview")
    expect(agentSkill).toContain('persona-harness/display-name: "(PH) Product Deep Interview"')
