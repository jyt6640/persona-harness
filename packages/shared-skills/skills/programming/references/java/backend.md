...
| Build | Gradle wrapper + Java toolchain; Gradle 9.x preferred; wrapper included as generated project output and preferred even when system Gradle exists | do not rely on a global build tool as the contract; never fake Gradle with Node/JS/Python/shell shims |
...
Generated Spring apps should prefer wrapper-backed verification. Include `gradlew`, `gradlew.bat`, and `gradle/wrapper` as generated project outputs when feasible; otherwise record why the wrapper could not be generated and avoid treating missing system Gradle as an application failure. Prefer the wrapper even when system Gradle exists because it makes a clean checkout reproducible. Use `./gradlew test` and `./gradlew build` on macOS/Linux, and `gradlew.bat test` and `gradlew.bat build` on Windows before using a global `gradle` command.

For Spring Boot Gradle dependency management, use the `org.springframework.boot` plugin plus `io.spring.dependency-management` or Boot plugin-managed dependencies. Do not put empty versions or dot versions on `spring-boot-starter-*`, JDBC/JPA/validation starters, or `org.flywaydb:flyway-core`. If the dependency version is unknown, follow a Spring Initializr/build.gradle template or the valid build line from the generated wrapper project. If `gradlew.bat test/build` or wrapper-backed test/build fails during dependency resolution, fix `build.gradle` and rerun verification instead of reporting success.
