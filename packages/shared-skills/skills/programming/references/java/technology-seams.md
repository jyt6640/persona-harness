...
| **Build** | **Gradle wrapper 9.x**, JDK pinned via a toolchain; wrapper first when system Gradle is absent | Gradle 8.14+ for Boot 4 compatibility, Maven only if the project harness chooses it | pin the JDK and build tool so CI and local builds agree; never replace Gradle with a fake Node/JS/Python/shell shim |
...
- prefer wrapper-backed verification: `./gradlew test` on macOS/Linux, `./gradlew.bat test` or `gradlew.bat test` on Windows; if system Gradle is missing, create/use the wrapper before using global `gradle`;
