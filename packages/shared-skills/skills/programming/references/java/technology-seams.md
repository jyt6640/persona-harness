...
| **Build** | **Gradle wrapper 9.x**, JDK pinned via a toolchain; wrapper included as generated project output and preferred even when system Gradle exists | Gradle 8.14+ for Boot 4 compatibility, Maven only if the project harness chooses it | pin the JDK and build tool so CI and local builds agree; never replace Gradle with a fake Node/JS/Python/shell shim |
...
- prefer wrapper-backed verification: `./gradlew test` and `./gradlew build` on macOS/Linux, `gradlew.bat test` and `gradlew.bat build` on Windows; include `gradlew`, `gradlew.bat`, and `gradle/wrapper/` in generated project outputs, and prefer the wrapper even when system Gradle exists for clean-project reproducibility;
- do not create `tools/gradle-shim.js` or any Node/JS/Python/shell shim to mimic `gradle test`, `gradle build`, or `bootRun`; if neither wrapper nor system Gradle can execute, report the toolchain/environment issue and stop claiming successful Gradle verification;
- use the `org.springframework.boot` plugin plus `io.spring.dependency-management` or Boot plugin-managed dependencies for Spring Boot Gradle dependency management; do not put empty versions or dot versions on `spring-boot-starter-*`, JDBC/JPA/validation starters, or `org.flywaydb:flyway-core`; if the dependency version is unknown, follow a Spring Initializr/build.gradle template or the valid build line from the generated wrapper project, and if wrapper-backed test/build fails during dependency resolution, fix `build.gradle` and rerun verification;
