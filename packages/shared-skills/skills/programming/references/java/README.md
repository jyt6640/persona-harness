...
- For Spring Boot Gradle builds, use the `org.springframework.boot` plugin plus `io.spring.dependency-management` or Boot plugin-managed dependencies. Do not attach empty versions or dot versions to `spring-boot-starter-*`, JDBC/JPA/validation starters, or `org.flywaydb:flyway-core`. If the dependency version is unknown, follow a Spring Initializr/build.gradle template or the valid build line from the generated wrapper project. If wrapper-backed test/build fails during dependency resolution, fix `build.gradle` and rerun it instead of reporting success.
