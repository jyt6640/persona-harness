...
```bash
bash "<programming-skill-directory>/scripts/java/check-no-excuse-rules.sh" <changed .java paths>
./gradlew compileJava test check
```

Replace `<programming-skill-directory>` with the directory containing the loaded
programming SKILL.md, not the consumer working directory or the parent `skills/`
directory. The portable plugin includes this checker. Pass existing changed Java
files explicitly and keep the consumer working directory for project build gates.
...
For a standalone Java file without a build or test framework, keep a small Java
regression program and run it with the requested JDK using `javac --release`
and `java`. Failed checks must throw or exit unsuccessfully, not merely print a
failure. Do not introduce a build system just for this substitute. JShell may
help exploration, but its printed output alone does not replace a rerunnable
regression or prove that the verification process has exited.
...
Pure-text rules live in `scripts/java/check-no-excuse-rules.sh` relative to the
programming skill directory. Run after every Java edit session with the resolved
skill path described above.
