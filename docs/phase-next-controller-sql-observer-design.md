...
- Controller method body에 SQL-like string literal만 있으면 `INFO`, confidence `LOW`.
...
- literal-only finding은 `INFO/LOW`로 고정한다.
- SQL-like literal-only만으로 prompt/rule 보강 후보를 확정하지 않는다.
- `LOW` confidence는 추가 evidence가 필요하다는 observation으로만 취급한다.

## SQL-Like Literal Severity Decision

선택지 A: `WARN/LOW`

- 장점: Controller에 SQL 문자열이 보이면 주의 신호로 잡을 수 있다.
- 단점: 메시지 문자열, 로그, 문서 문자열, 테스트 설명을 과하게 `WARN` 처리할 수 있다.

선택지 B: `INFO/LOW`

- 장점: false positive를 줄이고, import/type/member evidence 없는 경우를 과장하지 않는다.
- 단점: 실제 SQL 직접 사용을 약하게 볼 수 있다.

결정: **B. SQL-like literal-only는 `INFO/LOW`로 기록한다.**

이유:

- 문자열 기반 observer에서 literal-only evidence는 타입이나 호출 근거가 없다.
- literal-only SQL text는 API 설명, log message, validation message일 수 있다.
- `INFO/LOW`는 "추가 evidence가 필요하다"는 observation으로 충분하다.
- prompt/rule 보강 후보는 `WARN/HIGH` 또는 반복되는 `WARN/MEDIUM` evidence가 있을 때만 검토한다.
...
- `INFO/LOW`: SQL-like literal-only evidence. 추가 evidence 없이는 rule/prompt 개선 후보로 확정하지 않는다.
...
- Controller에 SQL-like literal만 있으면 `INFO/LOW`.
- SQL-like literal-only만으로 prompt/rule 보강 후보를 확정하지 않는다.
...
- SQL-like literal-only finding은 `INFO/LOW`로 구현한다.
