...
Current active candidate: narrow backend-only `ph policy init` and policy overlay summary implementation after philosophy/policy overlay design. v0.2.1 local/tarball validation is complete enough for continued local use; public publish remains deferred.
...
- Philosophy priority is company/team policy, then personal philosophy, then Clean Code baseline. README, current prompt, accepted workflow plan, and project-profile choices remain the requirements plane. Automatic philosophy/policy injection is not productized yet.
