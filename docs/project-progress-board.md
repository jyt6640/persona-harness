...
Current active candidate: `persona-harness@0.3.1-alpha.1`. The candidate packages the report-only workflow diagnostics surface (`ph workflow check`, `ph doctor`, `ph smoke`, `ph feedback`, `ph evidence summary`, and `ph review backend-shape`) plus AI-facing workflow rails (`ph workflow guard implement`, `ph workflow guard final`, `ph workflow start implement`, and `ph workflow finish implement`). Humans should be able to ask OpenCode/Codex-style TUI in plain language while the agent runs the `ph` workflow commands. `persona-harness@0.3.1-alpha.0` was published during the alpha pilot, but `0.3.1-alpha.1` is the next candidate because it removes stale Roomescape step contract rules from public init/package output. This is not a stable support claim.
