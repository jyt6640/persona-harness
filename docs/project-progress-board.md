...
- v0.3.x top-level intent router MVP is implemented: `detectTopLevelIntent` classifies user language into one primary rail among requirements, debug, review, refactor, git, and programming plus optional secondary intents. Requirements workflow injection now goes through this router so README-related debug/review/refactor/git requests do not automatically fall into implementation workflow. Dedicated debug, review, and refactor workflow blocks are active; git workflow block remains future scope.
- v0.3.x debug rail block is implemented for `debug` primary intent: failure reproduction, at least three hypotheses, evidence recording, confirmed-root-cause-only fixes, and test/build/smoke rerun are injected before the user's debug request. Review rail block is implemented for `review` primary intent: findings-first review, no code modification by default, file/line/evidence/impact findings, and explicit residual-risk reporting are injected before the user's review request. Refactor rail block is implemented for `refactor` primary intent: baseline public behavior evidence, no feature additions, small behavior-preserving structural changes, and same test/build/smoke rerun are injected before the user's refactor request. Git rail prompt is drafted in `docs/current/next-rail-prompt-drafts.md`; its runtime block remains future scope.
