...
11. `[~]` v0.3.1 workflow diagnostics and external tester surface
   - Evidence: `docs/current/v0.3.1-workflow-diagnostics-surface.md`, `docs/evidence-reviews/v0.3.1-short-tui-workflow-smoke.md`, `docs/evidence-reviews/v0.3.1-review-report-short-tui-smoke.md`, `docs/evidence-reviews/v0.3.1-workflow-guard-clean-tui-smoke.md`
   - Current state: candidate implementation adds report-only commands for workflow status, install diagnostics, smoke report, feedback template, evidence summary, and backend shape observation. `ph history` includes evidence-summary content when present, and injection text drops full shared-skill file paths while preserving metadata evidence. Clean short-TUI smoke confirmed that a minimal `README 보고 구현해줘` request used `npx ph plan --implement`, `npx ph workflow check`, filled implementation and review reports, ran `npx ph plan --report-filled implementation`, and ran `npx ph plan --report-filled review`. Backend shape was manually strong in the follow-up Study Room Booking run, and the matcher now reports the same run as all PASS after recognizing `*Store.java implements *Repository` adapters and verification evidence split across implementation/review reports. `ph workflow guard implement/final` now turn the implementation-start and final-answer workflow state into strict AI-facing gates while leaving `ph workflow check` report-only. The three-run guard smoke is partial: 3/3 runs entered the guard and plan path, 3/3 filled both workflow reports, but only 1/3 reached final guard PASS because command discipline around raw shell versus `npx ph bearshell` remained fragile.
   - Next candidate: add AI-facing workflow runner commands such as `ph workflow start implement` and `ph workflow finish implement`, reusing the existing lower-level guards. Do not turn backend shape or generated app quality into an enforcement gate.
