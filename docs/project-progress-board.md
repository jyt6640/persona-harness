...
- v0.3.1 candidate is workflow diagnostics, not broader Java rule expansion: short OpenCode TUI requests already show useful Java/Spring product-code shape influence, but can skip workflow evidence discipline. The next surface adds `ph workflow check`, `ph doctor`, `ph smoke`, `ph feedback`, `ph evidence summary`, and `ph review backend-shape` as report-only local commands, improves `ph history` archive summary with evidence-summary content, and strengthens `ph plan --implement` / `ph plan --prompt` so short implementation requests are routed through accepted plan, workflow status, `npx ph bearshell`, filled implementation report steps, and final review-report completion. Injection block dieting now removes full shared-skill reference paths from model-facing text while keeping metadata evidence intact.
- v0.3.1 short TUI workflow smoke is now positive for workflow report completion after follow-up: in clean installed tarball projects, the prompt `README 보고 구현해줘` triggered `npx ph plan --implement`, `npx ph workflow check`, implementation report filling, `npx ph plan --report-filled implementation`, review report filling, and `npx ph plan --report-filled review`; final `npx ph workflow check` reported `PASS` in the follow-up smoke. Generated backend shape was strong by manual inspection, with domain-first packages, domain repository ports, infrastructure adapters, class-based domain behavior, DTO boundaries, no Service-owned storage/id sequence, `bootJar` kept enabled, Gradle verification, and HTTP smoke. Remaining gaps are explicit: the follow-up run still used raw shell instead of `npx ph bearshell`, and `ph review backend-shape` has narrow matcher noise around `*Store.java` infrastructure adapters and verification-report text.
...
| v0.3.1 Diagnostics | Candidate | `ph workflow check`, `ph doctor`, `ph smoke`, `ph feedback`, `ph evidence summary`, and `ph review backend-shape` are being added as local report-only commands for external tester friction, workflow evidence discipline, evidence noise reduction, and backend shape observation. |
...
11. `[~]` v0.3.1 workflow diagnostics and external tester surface
   - Evidence: `docs/current/v0.3.1-workflow-diagnostics-surface.md`, `docs/evidence-reviews/v0.3.1-short-tui-workflow-smoke.md`, `docs/evidence-reviews/v0.3.1-review-report-short-tui-smoke.md`
   - Current state: candidate implementation adds report-only commands for workflow status, install diagnostics, smoke report, feedback template, evidence summary, and backend shape observation. `ph history` includes evidence-summary content when present, and injection text drops full shared-skill file paths while preserving metadata evidence. Clean short-TUI smoke confirmed that a minimal `README 보고 구현해줘` request used `npx ph plan --implement`, `npx ph workflow check`, filled implementation and review reports, ran `npx ph plan --report-filled implementation`, ran `npx ph plan --report-filled review`, and ended with `ph workflow check` PASS. Backend shape was manually strong in the follow-up Study Room Booking run: domain-first packages, domain repository ports, infrastructure adapters, class-based domain behavior, DTO boundaries, no Service-owned storage/id sequence, Gradle verification, bootRun, and HTTP smoke. Remaining gaps are narrower: the follow-up run still used raw shell rather than `npx ph bearshell`, and `ph review backend-shape` has matcher noise around `*Store.java` adapter names and verification report text.
   - Next candidate: either narrow-fix backend-shape adapter matching before the next alpha, or package v0.3.1 if the remaining `bearshell` preference is acceptable as guidance rather than a gate.
