...
- v0.3.x top-level intent router MVP is implemented: `detectTopLevelIntent` classifies user language into one primary rail among requirements, debug, review, refactor, git, and programming plus optional secondary intents. Requirements workflow injection now goes through this router so README-related debug/review/refactor/git requests do not automatically fall into implementation workflow. Dedicated requirements, debug, review, refactor, git, and programming workflow blocks are active; runtime rail text now loads from PH-owned workflow skill reference files; injected workflow rails write `phase0.intent.1` evidence with the user prompt, primary intent, secondary intents, reason, and rail marker.
