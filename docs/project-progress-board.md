...
11. `[~]` v0.3.1 workflow diagnostics and external tester surface
   ...
   - Current state: candidate implementation adds report-only commands for workflow status, install diagnostics, smoke report, feedback template, evidence summary, and backend shape observation. `ph history` includes evidence-summary content when present, and injection text drops full shared-skill file paths while preserving metadata evidence. Clean short-TUI smoke confirmed that a minimal `README 보고 구현해줘` request used `npx ph plan --implement`, `npx ph workflow check`, filled implementation and review reports, ran `npx ph plan --report-filled implementation`, ran `npx ph plan --report-filled review`, and ended with `ph workflow check` PASS. Backend shape was manually strong in the follow-up Study Room Booking run, and the matcher now reports the same run as all PASS after recognizing `*Store.java implements *Repository` adapters and verification evidence split across implementation/review reports. Remaining gap: the follow-up run still used raw shell rather than `npx ph bearshell`.
   - Next candidate: package v0.3.1 if the remaining `bearshell` preference is acceptable as guidance rather than a gate; otherwise add a report-only command-discipline note before packaging.
