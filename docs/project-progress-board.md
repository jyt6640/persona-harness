...
Current active candidate: `persona-harness@0.3.1-alpha.0`. The candidate packages the report-only workflow diagnostics surface (`ph workflow check`, `ph doctor`, `ph smoke`, `ph feedback`, `ph evidence summary`, and `ph review backend-shape`) and clarifies that `ph` is primarily an AI-facing workflow command surface. Humans should be able to ask OpenCode/Codex-style TUI in plain language while the agent runs the `ph` workflow commands. `persona-harness@0.3.0-alpha.3` is currently published on both the `alpha` and `latest` dist-tags during the alpha pilot. This is not a stable support claim.
...
| v0.3.1 Diagnostics | Alpha candidate | `ph workflow check`, `ph doctor`, `ph smoke`, `ph feedback`, `ph evidence summary`, and `ph review backend-shape` are implemented as local report-only commands for external tester friction, workflow evidence discipline, evidence noise reduction, and backend shape observation. README/release notes now frame `ph` as the AI-facing command surface that agents should call from short TUI implementation requests. |
