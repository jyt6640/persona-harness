...
- [Package version index](package-index.md): chronological package/version
  timeline from undocumented early rows through the current `0.6.0-rc.2` prep.
- [`v0.6.0-rc.2`](v0.6.0-rc.2/README.md): release-prep capsule for the next
  prerelease candidate. No registry smoke exists yet; `latest` remains
  `0.5.0` and `next` remains `0.6.0-rc.1` until publish.
