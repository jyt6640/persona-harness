...
| `0.8.0-beta.22` | 2026-08-02 | Consumer Authority Beta procedure-only source preparation | [`release notes`](../current/release/v0.8.0-beta.22-release-notes.md), [`beta lifecycle`](../current/release/consumer-authority-beta.md) | source-bound bootstrap before one final fixture commit; the same unpushed commit is the installed consumer CWD/HEAD for the one future hosted observation |
