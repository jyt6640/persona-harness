...
| `0.8.0-beta.6` | 2026-07-27 | Consumer Authority Beta source preparation | [`release notes`](../current/release/v0.8.0-beta.6-release-notes.md), [`beta lifecycle`](../current/release/consumer-authority-beta.md) | strict staging-only prerelease source candidate; requires source and packed public Java/Spring lifecycle, checksum-bound native descriptor traversal with no external descriptor opens, tag-bound publish/readback, current-version original signed artifact, and independent consumer verification |
