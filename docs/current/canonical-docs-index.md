...
| 0.8.0-beta.17 Consumer Authority Beta | `docs/current/release/v0.8.0-beta.17-release-notes.md` | Current staging-only source-preparation record for fixed external artifact transport, canonical no-token attestation grammar, separate caller/reusable binding, and one future original-artifact residual. |
| 0.8.0-beta.17 acceptance record | `docs/current/release/consumer-authority-beta17-acceptance.json` | Machine-readable source/packed lifecycle, package-root, bounded command/transport parser preflights, safe ZIP handoff, modeled exact-once Finish/replay, privacy, and one future current-version artifact residual. |
