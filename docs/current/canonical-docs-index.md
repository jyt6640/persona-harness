...
| 0.7.0-rc.7 prepared release notes | `docs/current/release/v0.7.0-rc.7-release-notes.md` | Staging-first source candidate with a packaged read-only exact-artifact verifier; no tag, package, registry, promotion, Stable/GA, or `latest` fact. RC6 registry facts cannot bind to this fresh source identity. |
