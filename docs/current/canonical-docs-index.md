...
| 0.8.0-beta.16 Consumer Authority Beta | `docs/current/release/v0.8.0-beta.16-release-notes.md` | Current staging-only source-preparation record for the canonical no-token external attestation command plan, separate caller/reusable binding, and one future original-artifact residual. |
| 0.8.0-beta.16 acceptance record | `docs/current/release/consumer-authority-beta16-acceptance.json` | Machine-readable source/packed lifecycle, package-root, bounded gh parser-preflight, modeled exact-once Finish/replay, privacy, and one future current-version artifact residual. |
