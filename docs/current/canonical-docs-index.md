...
| 0.8.0-beta.18 Consumer Authority Beta | `docs/current/release/v0.8.0-beta.18-release-notes.md` | Current staging-only source-preparation record for the Node20 canonical tar to isolated Node24 trusted-publisher handoff, with the same canonical-tar dry-run and real registry argv. |
| 0.8.0-beta.18 acceptance record | `docs/current/release/consumer-authority-beta18-acceptance.json` | Machine-readable source/packed lifecycle, package-root, portable package identity, bounded observer plans, and strict Node20 packer to Node24 publisher handoff with one hosted registry PUT residual. |
