...
| 0.8.0-beta.15 Consumer Authority Beta | `docs/current/release/v0.8.0-beta.15-release-notes.md` | Current staging-only source-preparation record for explicit initialized Finish state, separate caller/reusable binding, and one future original-artifact residual. |
| 0.8.0-beta.15 acceptance record | `docs/current/release/consumer-authority-beta15-acceptance.json` | Machine-readable source/packed public initialization, modeled exact-once Finish, replay, privacy, and one future current-version artifact residual. |
