...
| 0.8.0-beta.12 Consumer Authority Beta | `docs/current/release/v0.8.0-beta.12-release-notes.md` | Current staging-first source-preparation record for a bounded host-only observer credential-preflight; it makes no live publish, tag, release, promotion, or Finish authority claim. |
| 0.8.0-beta.12 acceptance record | `docs/current/release/consumer-authority-beta12-acceptance.json` | Machine-readable source/packed lifecycle, beta.11 historical preflight outcome, fixed observer credential isolation, and the single future hosted artifact residual. |
