...
Windows Korean stdin is now fixed at code level in `dd1e68f`:
...
- fix shape: raw buffer decode, UTF-8 first, then Windows Korean candidates
...
- this does not repair already broken artifacts

Publish-readiness still needs a published-package smoke after `dd1e68f`.
