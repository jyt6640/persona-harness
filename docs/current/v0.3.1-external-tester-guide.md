...
convention registry. Current HEAD through `a9bf926` routes closure blockers
...
`sg`/`ast-grep` skips with a warning instead of faking a pass. BYO targeted
smoke is PARTIAL because `observe --json` did not emit BYO ast-grep
