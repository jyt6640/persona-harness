...
- Gradle을 Java/Spring canonical build path로 둔다.
- Clean Code와 backend 역할 책임을 기본 baseline으로 둔다.
...
- Maven 기반 primary fixture/evidence를 유지하지 않는다
- 개인/팀/프로젝트 철학을 항상 강제하는 profile/intake harness
- H2/JdbcTemplate/schema.sql 같은 특정 persistence stack의 보편 강제
- RestAssured/MockMvc 같은 테스트 스타일 정책 확정

## 성공 기준

- `npm run test:repository`, `npm run typecheck`, `npm run build`가 통과한다.
...
세부 기준은 [phase0 step 2 scope](../phases/phase0/phase0-step2-scope.md)에 둔다.
...
`example/src`는 이 fixture 흐름에서 얻은 backend product code style reference answer다. 예제의 핵심은 roomescape나 step label이 아니라, Clean Code 기반으로 읽기 쉽고 리팩토링하기 쉬운 backend code flow가 균일하게 나오게 하는 방향이다. 특정 persistence stack, endpoint field, test style은 사용자의 프로젝트 요구사항과 선택에 따라 달라질 수 있다.
