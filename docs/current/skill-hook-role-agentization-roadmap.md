...
- `.persona/workflow/roles.md` generated by `ph workflow roles` and `ph plan` for non-autonomous blackbear/Charles/jaeki/roach role boundaries.
...
Status: implemented as non-autonomous role boundary artifact.
...
`ph workflow roles` writes and prints `.persona/workflow/roles.md`. `ph plan` also creates it with the other workflow artifacts. These should not become autonomous multi-agent behavior until the single-agent workflow rails are reliable.
