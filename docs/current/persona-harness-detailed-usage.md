...
아직 alpha publish 전이면 아래 local path flow를 사용한다.
...
`npx ph init`이 직접 만드는 핵심 파일은 다음이다.

- `.persona/harness.jsonc`
- `.persona/rules/`
- `.opencode/opencode.json`

`npx ph bootstrap backend`까지 실행하면 backend-ready workflow를 위해 다음 파일도 준비된다.
...
`npx ph intake --interactive`는 선택 단계다. 구현 전에 프로젝트 맥락, 목적, 규모, 애플리케이션 형태, 저장소 선택, persistence, migration, package/architecture style, boundary strictness, project note를 한 질문씩 받아 `.persona/project-profile.jsonc`에 저장한다. `npx ph intake`만 실행하면 같은 schema의 editable draft를 만든다. 현재는 planning surface이며 rule enforcement나 generated app product-quality 보증이 아니다.
...
구현/검토 report를 실제로 채운 뒤에는 `npx ph plan --report-filled implementation` 또는 `npx ph plan --report-filled review`로 해당 report의 상태를 `filled`로 바꾼다. 이건 local evidence metadata이며 build/test gate나 품질 보증이 아니다.
...
실제 OpenCode 실행 중 hook이 발동하면 아래 경로에 privacy-classified
evidence가 남는다. 기본 canonical mode는 `safe_metadata`이며, 기존
`metadata_only` 값은 같은 동작으로 normalize되는 deprecated compatibility
alias다.
...
`evidenceMode` 선택지는 다음과 같다.

- `safe_metadata`: 기본값. raw prompt, full argv, stdout/stderr/tool output을
  저장하지 않고 count, SHA-256, fixed diagnostic signal, opaque reference만
  저장한다.
- `redacted_diagnostics`: execution 진단 preview를 secret redaction 후 bounded
  길이로 추가한다.
- `prompt_diagnostics`: execution 진단과 prompt 진단 preview를 모두
  redaction/bounding 후 저장하는 명시적 opt-in이다.
- `metadata_only`: `safe_metadata`로 normalize되는 deprecated alias다.

각 record의 privacy class는 `metadata-safe`,
`redacted-execution-diagnostics`, `redacted-prompt-diagnostics`, 또는
`trusted-attestation-metadata`다. 마지막 값은 attestation metadata의
privacy 분류일 뿐 finish authority를 부여하지 않는다.

POSIX에서는 evidence directory/file mode를 각각 `0700`/`0600`으로
enforce한다. Windows에서는 POSIX mode를 주장하지 않으며 동일한 write
경로를 사용하되 ACL hardening은 별도 지원 범위다.
