...
### Post-publish repo-side eval procedure note
...
After `7d23167 fix(eval): match current opencode run surface`, the default runner command is:
...
This is still a runner/OpenCode invocation failure, not PH product evidence, not PH value verdict, and not generated app quality evidence. Actual successful generation pilot is still pending.

Use only real `results.json` and capture from successful generation for replay/decide. Do not hand-enter PASS results or fake metrics.
...
- The post-`7d23167` QA rerun failed on OpenCode invocation shape before useful generation, so a successful generation pilot is still pending.
