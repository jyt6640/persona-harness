...
- Replay decide: FAIL.
  ...
  - PH ON failure-mode reduction was `-75%`, below the 20% threshold.
- Verdict is stable FAIL; reason text differs only because strongest-OFF tie/order labeling differs.
...
- PH ON r2 runtime smoke left an orphan Gradle/Java process; QA cleaned only that eval workspace path.
...
- It reports real post-publish ON/OFF eval pilot results: a replay-fixed `n=1` coded gate PASS for supplied results, followed by a 2-rep stable FAIL that blocks full-matrix expansion readiness.
...
- A real ON/OFF eval pilot exists and replay-fixed decide PASSed for supplied `n=1` results, but the 2-rep follow-up failed the coded gate and blocks full-matrix expansion readiness.
...
- The 2-rep pilot showed PH ON stack alignment improvement but runtime/failure-mode regression versus OFF baselines.
