...
- It reports a real post-publish ON/OFF eval pilot result and replay-fixed `n=1` coded gate PASS for supplied results.
- Published alpha3 registry smoke is package surface smoke only.
- Latest-tag smoke is also install/surface smoke only.
- Registry/latest install smoke did not include a generated-app OpenCode run; the later repo-side eval pilot did generate apps, and its replay-fixed `n=1` coded gate result is PASS for supplied results.
...
- A real ON/OFF eval pilot exists and replay-fixed decide PASSed for supplied `n=1` results.
...
- Replay still does not preserve provider timeout/providerFailed or original PH ON workflow finish outcome exactly.
