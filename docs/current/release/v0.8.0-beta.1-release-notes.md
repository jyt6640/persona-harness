...
- Publish readback independently binds the fixed npm package/version, selected
  ...
  treating generic npm signature output as provenance.
- The packaged `ph authority` surface has explicit user-scoped public GitHub
  enrollment and original-artifact fetch commands. It never accepts
  caller-local evidence as authority and it never consumes Finish authority.
