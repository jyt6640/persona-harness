...
provenance artifact, or dist-tag change has been created for `0.7.0-rc.5`.
Current published channels remain `latest=0.6.0` and `next=0.7.0-rc.3`.

## Summary

`0.7.0-rc.5` prepares a new immutable protected-main source identity after the
untagged, unpublished RC4 source preparation. Persona Harness remains an AI
...
- The controlled staged-package producer preserves exact source binding. A
  registry gitHead recorded for RC4 cannot bind to RC5; any future RC5 staging
  tarball must carry RC5's own exact protected-main source identity.
...
- No `v0.7.0-rc.5` tag, GitHub prerelease, npm package, registry package
...
4. If staging is later authorized, retain the exact source, registry, tarball,
   ...
   path.
5. Create `v0.7.0-rc.5` only through the approved canonical-main release
