...
and rejected Linux's runtime-owned `UV_USE_IO_URING=0` child marker. The next
strict-prerelease procedure-only source candidate is `0.8.0-beta.26`; it has no
source-level claim of a tag, publish, channel movement, GitHub release, or
original signed consumer artifact.

The beta26 release contract requires package-visible observer credential
...
are never allowlisted. The observer `gh` is a workflow-selected private absolute
regular non-symlink compatible executable, never an ambient PATH lookup or
literal runner path. Linux accepts
