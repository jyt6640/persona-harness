...
strict-prerelease procedure-only source candidate is `0.8.0-beta.25`; it has no
source-level claim of a tag, publish, channel movement, GitHub release, or
original signed consumer artifact.

The beta25 release contract requires package-visible observer credential
...
are never allowlisted. The observer `gh` is an explicit absolute regular
non-symlink compatible executable, never an ambient PATH lookup. Linux accepts
