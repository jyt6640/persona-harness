...
- Package evidence must begin from a cloneable exact-HEAD bundle that also
  records `refs/remotes/origin/main`; a clean checkout then binds package and
  lock metadata, npm pack metadata, built CLI version, and fresh installed CLI
  version before any registry or authority evidence is considered.
