...
- `latest` dist-tag sync remains a separate OTP-protected release-close step.
...
Do not reopen broad A/B loops unless a new clean smoke regresses on package-flow or `bootJar`. Finish `latest` dist-tag sync and GitHub release closure first.
