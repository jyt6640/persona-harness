...
- `ph bootstrap backend --strict` initializes only absent empty lifecycle loop
  ...
  init transaction, it reserves canonical project-contained `.persona` and
  `.persona/workflow` directories before modifying config, profile, policy,
  plan, role, report, or loop-state artifacts. Parent or leaf aliases and
  detected replacements block with no bootstrap write outside the project.
