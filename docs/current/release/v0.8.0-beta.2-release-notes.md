...
- `ph bootstrap backend --strict` initializes only absent empty lifecycle loop
  ...
  init transaction, it reserves a canonical project-contained `.persona/workflow` path with no-follow semantics.
  It does so before modifying config, profile, policy, plan, role, report, or
  loop-state artifacts. Parent or leaf aliases and detected replacements block
  with no bootstrap write outside the project.
