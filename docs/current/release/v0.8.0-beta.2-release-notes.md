...
- `ph bootstrap backend --strict` initializes only absent empty lifecycle loop
  states. It leaves malformed or stale state fail-closed, and blocks parent or
  leaf aliases and detected replacements through a canonical project-contained `.persona/workflow` path with no-follow state reservation before writing either loop-state file.
