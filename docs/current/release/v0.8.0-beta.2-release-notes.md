...
- `ph bootstrap backend --strict` initializes only absent empty lifecycle loop
  ...
  project-root reservation is established, fresh `.persona` initialization is
  assembled outside the caller workspace and promoted only while its target is
  still absent. Bootstrap then reserves canonical project-contained `.persona`
  and `.persona/workflow` paths with no-follow semantics before modifying
  config, profile, policy, plan, role, report, or loop-state artifacts. Parent
  or leaf aliases and detected replacements block with no bootstrap write
  outside the project.
