...
Current decision: `candidate-not-cut`

`0.3.0-alpha.3` is a reasonable next alpha only after the resume/next workflow gap is closed or explicitly deferred.
...
- optionally `ph plan --next` / `ph plan --resume` if completed before the release
...
Cut `0.3.0-alpha.3` when one of these is true:

1. `ph plan --next` or `ph plan --resume` exists and has a smoke test, so interrupted TUI implementation runs have a recovery path.
2. The release notes clearly state that alpha.3 only ships report/CLI hardening, and TUI resume remains a known gap.

If neither is true, keep the version at `0.3.0-alpha.2` and continue implementation.
