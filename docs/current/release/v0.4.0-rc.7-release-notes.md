...
`0.4.0-rc.7` is a next-channel prerelease candidate for the default OpenCode
developer MCP bundle and external CodeGraph wrapper surface. Do not move
`latest`.

Before publish, registry `persona-harness@next` still points at
`0.4.0-rc.6` with gitHead
...
This is a next-channel prerelease candidate only. Until post-publish registry
verification confirms rc7 gitHead, the evidence above remains local/current
tarball package-runtime surface evidence.

Do not claim:

- registry `@next` rc7 behavior before `npm view` verifies gitHead;
...
Publish with `next`, not `latest`.

After publish, verify:

```bash
npm view persona-harness@0.4.0-rc.7 version gitHead dist.shasum --json
npm dist-tag ls persona-harness
```

Then run a registry-only package-surface smoke before recording registry facts.
