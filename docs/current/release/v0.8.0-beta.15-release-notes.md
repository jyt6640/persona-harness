...
One authoritative-bundle command materializes fresh target and base checkouts,
rejects a stale launcher before pack, verifies resolved manifest and lock paths,
and runs the built source CLI plus fresh installed consumer package exercise
against the exact target tarball SHA-256. The package exercise uses the same
