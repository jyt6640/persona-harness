...
[`consumer-authority-beta33-acceptance.json`](consumer-authority-beta33-acceptance.json).
Every record has only `schemaVersion`, `surface`, `phase`, `state`, and the
fixed bounded code. The clean-bundle parent accepts an exact complete ready
transcript plus one exact terminal PASS marker. A marker-only, malformed,
out-of-order, foreign, or success-after-blocked transcript becomes a bounded
phase-envelope block.
