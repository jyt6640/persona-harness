...
- Added strict AI-facing workflow gates: `ph workflow guard implement` and `ph workflow guard final`.
...
- Tightened short implementation guidance so agents should run `npx ph workflow guard implement`, `npx ph plan --implement`, `npx ph bearshell`, `npx ph plan --report-filled ...`, and `npx ph workflow guard final`.
...
- Validate the strict `ph workflow guard implement/final` path in the next clean OpenCode TUI smoke.
