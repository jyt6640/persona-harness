...
- The registry centralizes convention id, default level, blocker id, step id,
  fix path, `blockAllowed`, and `highPrecision` metadata across check, closure,
  continue, finish, archive, and write-warning surfaces. BYO `observe --json`
  emission is not yet aligned.
...
- External targeted smoke is PARTIAL for BYO surface alignment: BYO
  ...
  emit BYO ast-grep `controller.persistence-import`.
