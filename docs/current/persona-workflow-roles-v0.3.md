...
- plan proposed
- plan accepted or revised through `ph plan --accept` / `ph plan --revise`
...
After `.persona/workflow/plan.md` is generated by `ph plan`, run a clean OpenCode smoke where the model fills the plan, leaves an explicit accepted or needs-revision marker, and only then proceeds to implementation.
