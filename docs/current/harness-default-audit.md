...
| `features.sharedSkillRouting` | UX | `true` | `true` | unchanged | **`true`** | Compact OpenCode shared-skill advisory route. It is independent from model-facing runtime context injection, selects at most one reference, and respects an explicit `false` opt-out. | `src/config/harness-config.ts`; `src/runtime/hooks.ts`; `src/runtime/opencode-shared-skill-routing-status.ts` |
