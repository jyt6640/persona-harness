...
| `features.entrySteering` | UX opt-in | `false` | `false` | unchanged | **`false`** | Default-off OpenCode first-message advisory; separate from measured-negative runtime injection and never an automatic workflow transition. | `docs/current/entry-steering-status.md`; STEP 4 candidate source |
