...
This is a local/tarball/GitHub install pilot. It is not a public npm release test.
...
## Install Option A: GitHub Repo Install

Use this when the tester can install directly from the repository.

```bash
mkdir -p /tmp/persona-harness-external-check
cd /tmp/persona-harness-external-check
npm init -y
npm install -D github:<owner>/persona-harness#<commit-sha>
npx ph init
...
Replace `<owner>` and `<commit-sha>` with the actual repository owner and commit being tested.

## Install Option B: Tarball Install

Use this when sending a `.tgz` file directly.

Create the tarball from the Persona Harness repository:
...
Then install it in the tester's clean project:
