...
| `docs/current/release/v0.8.0-beta.3-release-notes.md` | version-specific release note | v0.8.0-beta.3 | Current Consumer Authority Beta source-preparation record for a strict staging-only prerelease; it makes no live publish, tag, release, promotion, or Finish authority claim. |
| `docs/current/release/consumer-authority-beta3-acceptance.json` | machine-readable release contract | v0.8.0-beta.3 | Fixed source/packed public lifecycle commands, adversarial classes, and the single hosted current-version artifact residual. |
