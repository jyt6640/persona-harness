...
| `docs/current/external-attested-finish-walkthrough.md` | current active pointer/status | - | Consumer-facing walkthrough of the external-attested finish path, written from an end-to-end run: setup, the enroll/fetch/finish/replay sequence, and the three undocumented traps that stopped it. Records one run on one day; not a support claim and not a quality claim. |
| `docs/current/first-completion-path-measurement.md` | current active pointer/status | - | Counts the refusals between install and a first verified completion — twelve, eight of them undocumented at the time — from one end-to-end run. One operator, one day; states plainly that abandonment, wall-clock time, and distribution are not measurable this way. |
...
| `docs/current/observer-precision-on-real-codebases.md` | current active pointer/status | - | Hand-verified observer precision against two unmodified public Spring projects (549 Java files); records 14/14 correct warnings and the two false positives the run removed. Precision only on a two-project sample, and the observers have an opinion about 19% of files — not a recall or coverage claim. |
...
| `docs/current/persona-shared-skills-core.md` | current active pointer/status | - | Canonical portable Persona shared-skill catalog, product interview, host-adapter, and package-surface contract. |
...
| `docs/current/release/v0.8.0-beta.2-release-notes.md` | version-specific release note | v0.8.0-beta.2 | Immutable staging-only Consumer Authority Beta evidence; it is not current-version consumer authority, promotion, Stable/GA, or Finish authority evidence. |
| `docs/current/release/v0.8.0-beta.3-release-notes.md` | version-specific release note | v0.8.0-beta.3 | Historical immutable staging-only Consumer Authority Beta evidence; it is not beta.12 current-version consumer authority, promotion, Stable/GA, or Finish authority evidence. |
| `docs/current/release/v0.8.0-beta.5-release-notes.md` | version-specific release note | v0.8.0-beta.5 | Historical Consumer Authority Beta source-preparation record; it makes no beta.12 current-version consumer authority, promotion, Stable/GA, or Finish authority claim. |
| `docs/current/release/v0.8.0-beta.6-release-notes.md` | version-specific release note | v0.8.0-beta.6 | Historical staging-only Consumer Authority Beta evidence; it is not beta.12 current-version consumer authority, promotion, Stable/GA, or Finish authority evidence. |
| `docs/current/release/v0.8.0-beta.7-release-notes.md` | version-specific release note | v0.8.0-beta.7 | Historical immutable staging-only Consumer Authority Beta evidence. Its original artifact missed independent online verification inside its leaf certificate window and cannot satisfy beta.10 authority. |
| `docs/current/release/v0.8.0-beta.8-release-notes.md` | version-specific release note | v0.8.0-beta.8 | Historical Consumer Authority Beta staging evidence. Its original artifact verified live, but an obsolete caller workflow prearm retained no authority artifact and cannot satisfy beta.12 authority. |
| `docs/current/release/v0.8.0-beta.9-release-notes.md` | version-specific release note | v0.8.0-beta.9 | Historical Consumer Authority Beta staging evidence. Its original artifact verified live, but the installed fetch treated its enrolled workflow filename as a repository path and retained no authority artifact. |
| `docs/current/release/v0.8.0-beta.10-release-notes.md` | version-specific release note | v0.8.0-beta.10 | Historical Consumer Authority Beta staging evidence. Its original bytes and online crypto passed, but the isolated observer lacked a usable GitHub read credential for installed authority discovery. |
| `docs/current/release/v0.8.0-beta.11-release-notes.md` | version-specific release note | v0.8.0-beta.11 | Historical Consumer Authority Beta source-preparation record. Its observer credential pre-arm did not establish a verified isolated observer condition before fixture authorization and provides no reusable authority evidence. |
| `docs/current/release/v0.8.0-beta.12-release-notes.md` | version-specific release note | v0.8.0-beta.12 | Historical Consumer Authority Beta staging evidence. Original artifact/crypto/fetch preparation passed, but public Finish retained readiness blockers and it supplies no beta.15 authority, consumption, or replay evidence. |
| `docs/current/release/v0.8.0-beta.13-release-notes.md` | version-specific release note | v0.8.0-beta.13 | Historical Consumer Authority Beta staging evidence. Current artifact crypto passed, but installed fetch retained no authority at the bootstrap-local source binding and supplies no beta.15 authority, consumption, or replay evidence. |
| `docs/current/release/v0.8.0-beta.14-release-notes.md` | version-specific release note | v0.8.0-beta.14 | Historical Consumer Authority Beta staging evidence. Original artifact, crypto, and fetch passed, but an uninitialized consumer Finish made no authority decision. |
| `docs/current/release/v0.8.0-beta.15-release-notes.md` | version-specific release note | v0.8.0-beta.15 | Historical Consumer Authority Beta staging evidence. Its final external observer combined mutually exclusive GitHub CLI identity selectors before crypto and establishes no current-version authority evidence. |
| `docs/current/release/v0.8.0-beta.16-release-notes.md` | version-specific release note | v0.8.0-beta.16 | Historical Consumer Authority Beta staging evidence. Its observer accepted an exit-zero API download without proving a nonempty exact ZIP, so it supplies no beta.17 transport or authority evidence. |
| `docs/current/release/v0.8.0-beta.17-release-notes.md` | version-specific release note | v0.8.0-beta.17 | Historical Consumer Authority Beta source-preparation record. Its canonical tar/provenance observation ended at an authorization-shaped registry response and makes no package-absence, promotion, or Finish authority claim. |
| `docs/current/release/v0.8.0-beta.18-release-notes.md` | version-specific release note | v0.8.0-beta.18 | Historical immutable staging evidence. Registry bytes matched canonical facts, but an unsupported registry gitHead predicate blocked workflow readback. |
| `docs/current/release/v0.8.0-beta.19-release-notes.md` | version-specific release note | v0.8.0-beta.19 | Historical Consumer Authority Beta staging evidence; it makes no current-version authority, consumption, replay, promotion, or Finish authority claim. |
| `docs/current/release/v0.8.0-beta.22-release-notes.md` | version-specific release note | v0.8.0-beta.22 | Historical non-reusable procedure evidence: host gh/XDG state created an untracked `.local` entry after the final commit. |
| `docs/current/release/v0.8.0-beta.23-release-notes.md` | version-specific release note | v0.8.0-beta.23 | Current procedure-only Consumer Authority Beta candidate: v3 isolates all fifteen host-state roots outside the consumer realpath and verifies clean state after every pre-push stage. |
| `docs/current/release/consumer-authority-beta7-acceptance.json` | machine-readable release contract | v0.8.0-beta.7 | Historical source/packed lifecycle record retained with beta.7 staging evidence; it cannot satisfy beta.10 current-version authority. |
| `docs/current/release/consumer-authority-beta8-acceptance.json` | machine-readable release contract | v0.8.0-beta.8 | Historical source/packed lifecycle and beta.7 expiry record. The later beta.8 original-artifact discovery failure is recorded by the beta.9 acceptance contract. |
| `docs/current/release/consumer-authority-beta9-acceptance.json` | machine-readable release contract | v0.8.0-beta.9 | Historical source/packed lifecycle record for beta.8 discovery-to-binding; beta.9's independently verified original artifact and endpoint failure are recorded by the beta.10 acceptance contract. |
| `docs/current/release/consumer-authority-beta10-acceptance.json` | machine-readable release contract | v0.8.0-beta.10 | Historical source/packed discovery-to-binding contract retained with beta.10 staging evidence. |
| `docs/current/release/consumer-authority-beta11-acceptance.json` | machine-readable release contract | v0.8.0-beta.11 | Historical source/packed discovery-to-binding contract and bounded credential-handoff proposal; it does not supply beta.12 preflight or authority evidence. |
| `docs/current/release/consumer-authority-beta12-acceptance.json` | machine-readable release contract | v0.8.0-beta.12 | Historical source/packed observer-preflight contract; its successful artifact/fetch preparation did not establish public Finish readiness or beta.14 authority evidence. |
| `docs/current/release/consumer-authority-beta13-acceptance.json` | machine-readable release contract | v0.8.0-beta.13 | Historical source/packed public Java/Spring pre-authority readiness and credential-isolation contract; it did not establish beta.14 authority evidence. |
| `docs/current/release/consumer-authority-beta14-acceptance.json` | machine-readable release contract | v0.8.0-beta.14 | Source/packed public Java/Spring readiness and separate caller/reusable signer binding contract with bootstrap-local init metadata exclusion, privacy-safe output, and one future current-version artifact residual. |
| `docs/current/release/consumer-authority-beta15-acceptance.json` | machine-readable release contract | v0.8.0-beta.15 | Historical source/packed public initialization and authority-backed Finish readiness contract; it cannot replace beta.16 current-version crypto or authority evidence. |
| `docs/current/release/consumer-authority-beta16-acceptance.json` | machine-readable release contract | v0.8.0-beta.16 | Historical source/packed public initialization, root-bound exact-tar package proof, canonical no-token gh parser preflight, and modeled authority-backed Finish readiness contract. |
| `docs/current/release/consumer-authority-beta17-acceptance.json` | machine-readable release contract | v0.8.0-beta.17 | Source/packed public initialization, root-bound exact-tar package proof, canonical no-token command and transport parser preflights, safe external ZIP handoff, and authority-backed Finish readiness contract with modeled exact-once consumption/replay and one future current-version artifact residual. |
| `docs/current/release/consumer-authority-beta18-acceptance.json` | machine-readable release contract | v0.8.0-beta.18 | Source/packed lifecycle and package-root contract extended with the strict Node20 canonical packer, isolated Node24 publisher, exact canonical-tar dry-run, and one hosted registry PUT/readback residual. |
| `docs/current/release/consumer-authority-beta19-acceptance.json` | machine-readable release contract | v0.8.0-beta.19 | Historical source/packed lifecycle and package-root contract extended with workflow-bound canonical source-to-registry-byte reconciliation and sanitized failed-readback evidence. |
| `docs/current/release/consumer-authority-beta22-acceptance.json` | machine-readable release contract | v0.8.0-beta.22 | Historical procedure-only source/packed observer contract binding the beta22 final-observer record. |
| `docs/current/release/consumer-authority-beta23-acceptance.json` | machine-readable release contract | v0.8.0-beta.23 | Current v3 source/packed observer contract binding the immutable host-state isolation record, source-bound pre-push commit, and one hosted residual. |
