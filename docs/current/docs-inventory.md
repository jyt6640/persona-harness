...
| `docs/current/release/v0.8.0-beta.17-release-notes.md` | version-specific release note | v0.8.0-beta.17 | Current Consumer Authority Beta source-preparation record for fixed external artifact transport, canonical no-token command-plan grammar, initialized public Finish state, modeled exact-once consumption, and replay block; it makes no live publish, tag, release, promotion, or Finish authority claim. |
