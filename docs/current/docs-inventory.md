...
| `docs/current/release/v0.8.0-beta.16-release-notes.md` | version-specific release note | v0.8.0-beta.16 | Current Consumer Authority Beta source-preparation record for a canonical no-token external attestation command plan, initialized public Finish state, modeled exact-once consumption, and replay block; it makes no live publish, tag, release, promotion, or Finish authority claim. |
...
| `docs/current/release/consumer-authority-beta16-acceptance.json` | machine-readable release contract | v0.8.0-beta.16 | Source/packed public initialization, root-bound exact-tar package proof, canonical no-token gh parser preflight, and authority-backed Finish readiness contract with modeled exact-once consumption/replay and one future current-version artifact residual. |
