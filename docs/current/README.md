...
- Release operations and workflow-compatible release notes:
  ...
  [`release/v0.7.0-rc.5-release-notes.md`](release/v0.7.0-rc.5-release-notes.md).
...
- `0.7.0-rc.5` is source-prepared only as a staging-first prerelease
  candidate. It records no tag, GitHub prerelease, npm package, registry
  gitHead, integrity, provenance, or dist-tag fact. Its future registry
  gitHead must bind to its own exact protected-main source identity; RC4
  registry facts cannot authorize RC5. Published channels remain
  `latest=0.6.0` and `next=0.7.0-rc.3`; local or caller-provided
  staged-package evidence remains `artifact-provenance-unavailable`.
