...
Persona Harness `v0.3.0-alpha.1`:
...
Current package version: `0.3.0-alpha.1`
...
- `alpha.1` keeps the tester-facing alpha line and updates setup docs after the first publish.
...
- Tags matching `vX.Y.Z-alpha.N` publish with npm dist-tag `alpha`.
- Tags matching `vX.Y.Z-beta.N` publish with npm dist-tag `beta`.
...
Proceed toward `v0.3.0-alpha.1` npm alpha readiness.
