...
> Stable npm channel: `persona-harness@latest=0.5.0`; prerelease channel is `next=0.6.0-rc.1`; alpha remains `0.3.9-alpha.8`. The `0.6.0-rc.1` registry smoke is package-runtime evidence for prerelease surfaces only, not a `latest` move or broader product-efficacy claim.
