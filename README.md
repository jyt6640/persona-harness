...
> Stable npm channel: `persona-harness@latest=0.5.0`; published prerelease channel is `next=0.6.0-rc.3`; release prep target is `0.6.0-rc.4` until QA publish-readiness; alpha remains `0.3.9-alpha.8`. Stable `0.6.0` remains deferred, and the `0.6.0-rc.4` prep is not registry evidence until a future `next` publish and External registry smoke.
