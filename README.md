...
> Stable npm channel: `persona-harness@latest=0.5.0`; published prerelease channel remains `next=0.6.0-rc.2`; release prep target is `0.6.0-rc.3` until a separate publish; alpha remains `0.3.9-alpha.8`. The `0.6.0-rc.2` registry smoke is release-candidate package-runtime evidence only, not a `latest` move or broader product-efficacy claim.
