...
> 安定 npm チャンネルは `persona-harness@latest=0.5.0` です。prerelease チャンネルは現在 `next=0.6.0-rc.2`、alpha は `0.3.9-alpha.8` です。`0.6.0-rc.2` registry smoke は prerelease surface の package-runtime evidence であり、`latest` の移動やより広い product-efficacy claim ではありません。
