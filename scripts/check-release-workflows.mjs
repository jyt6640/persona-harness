...
const requirements = [
  ...
  ["project finish context diagnostic fixed binding", ".github/workflows/persona-harness-project-finish-context-diagnostic.yml", (text) => text.includes("persona-harness-project-finish-context-diagnostic.yml") && text.includes("CALLER_WORKFLOW_REF: ${{ github.workflow_ref }}") && text.includes("matching.length !== 1") && !text.includes("GITHUB_JOB:") && text.includes("ref: ${{ steps.producer-pin.outputs.sha }}") && text.includes("PERSONA_HARNESS_PRODUCER_SHA") && text.includes("node scripts/verify-project-finish-producer-checkout.mjs") && text.includes("PROJECT_FINISH_DIAGNOSTIC_CALLER_WORKFLOW_REF") && text.includes("PROJECT_FINISH_DIAGNOSTIC_CALLER_WORKFLOW_SHA") && text.includes("PROJECT_FINISH_DIAGNOSTIC_REUSABLE_WORKFLOW_REF") && text.includes("PROJECT_FINISH_DIAGNOSTIC_REUSABLE_WORKFLOW_SHA") && text.includes("PROJECT_FINISH_DIAGNOSTIC_OIDC_REQUEST_TOKEN") && text.includes("PROJECT_FINISH_DIAGNOSTIC_OIDC_REQUEST_URL") && text.includes("env -i") && !text.includes("$PATH") && !text.includes("$HOME") && !text.includes("$GIT_CONFIG_GLOBAL") && text.includes("scripts/diagnose-project-finish-producer-context.mjs")],
