...
const requirements = [
  ...
  ["release canonical published artifact readback", ".github/workflows/release.yml", (text) =>
    ...
    && text.indexOf("npm run test:repository") < text.indexOf('git checkout --detach "$tag_commit"')
    && text.includes('test "$tag_commit" = "$RELEASE_SOURCE_HEAD"')
    && text.indexOf('test "$tag_commit" = "$RELEASE_SOURCE_HEAD"') < text.indexOf('git checkout --detach "$tag_commit"')
    && text.includes("- name: Install tagged dependencies\n        run: npm ci")
    && text.indexOf('git checkout --detach "$tag_commit"') < text.indexOf("- name: Install tagged dependencies")
    && text.includes("run: npx --no-install vitest run --testTimeout=15000")
    && text.indexOf("- name: Install tagged dependencies") < text.indexOf("run: npx --no-install vitest run --testTimeout=15000")
    && text.indexOf("run: npx --no-install vitest run --testTimeout=15000") < text.indexOf("canonical-package-packer.mjs --output-directory")
  ...
  ["project finish attester reusable binding", ".github/workflows/persona-harness-project-finish.yml", (text) => text.includes("repository: jyt6640/persona-harness") && text.includes("id: producer-pin") && text.includes("CALLER_WORKFLOW_REF: ${{ github.workflow_ref }}") && text.includes("structuredJobs") && text.includes("ref: ${{ steps.producer-pin.outputs.sha }}") && text.includes("PERSONA_HARNESS_PRODUCER_SHA") && text.includes("node scripts/verify-project-finish-producer-checkout.mjs") && text.includes("uses: actions/github-script@ed597411d8f924073f98dfc5c65a23a2325f34cd") && text.includes("project-finish-attestation-producer-oidc-capability-bridge.cjs") && text.includes("runProjectFinishAttestationProducerWithCore({ core })") && !text.includes("ref: ${{ github.workflow_sha }}") && !text.includes("ruby <<") && !text.includes("run: node scripts/build-project-finish-attestation.mjs") && !text.includes("ACTIONS_ID_TOKEN_REQUEST_")],
  ["project finish attester caller isolation", ".github/workflows/persona-harness-project-finish.yml", (text) => text.includes("path: .project-finish-caller") && text.includes("working-directory: .project-finish-caller") && text.includes("PROJECT_FINISH_ATTESTATION_BUNDLE_PATH: ${{ steps.attest.outputs.bundle-path }}") && text.includes("node scripts/project-finish-attestation-artifact-handoff.mjs --collect") && text.includes("node scripts/project-finish-attestation-artifact-handoff.mjs --verify-signed") && !text.includes('run: cp "${{ steps.attest.outputs.bundle-path }}"') && text.includes("path: |\n            .project-finish-attestation-artifacts/receipt.json\n            .project-finish-attestation-artifacts/predicate.json\n            .project-finish-attestation-artifacts/bundle.json") && text.includes("path: .project-finish-attestation-failure/failure-diagnostic.json")],
  ["project finish attester subject", ".github/workflows/persona-harness-project-finish.yml", (text) => text.includes("project-finish-attestation.1") && text.includes("subject-path: .project-finish-attestation-artifacts/receipt.json") && text.includes("predicate-path: .project-finish-attestation-artifacts/predicate.json")],
  ["project finish attester boundary", ".github/workflows/persona-harness-project-finish.yml", (text) => !text.includes("npm publish") && !text.includes("git tag") && !text.includes("git push") && !text.includes("workflow finish") && text.includes("failure-diagnostic.json") && text.includes("if: always()") && text.includes("include-hidden-files: true") && text.includes("contents: read") && text.includes("id-token: write") && text.includes("attestations: write") && text.includes("artifact-metadata: write") && !text.includes("contents: write")],
...
async function main() {
  ...
  if (!hasPackageObserverResponsibility(packageManifest, currentAcceptanceManifest)) {
