...
import { canonicalV085AcceptanceManifest } from "./consumer-authority-v085-acceptance-schema.mjs"
...
const V086_PACKAGE_VERSION = "0.8.6"
const ACCEPTANCE_PATH = join("docs", "current", "release", "consumer-authority-v086-acceptance.json")
...
export function canonicalV086AcceptanceManifest() {
  return structuredClone(EXPECTED_MANIFEST)
}

export function readV086AcceptanceManifest(packageRoot) {
  let packageVersion
  let value
  try {
  ...
  return parseV086AcceptanceManifest(value, packageVersion)
}

export function parseV086AcceptanceManifest(value, packageVersion) {
  if (packageVersion !== V086_PACKAGE_VERSION || !isDeepStrictEqual(value, EXPECTED_MANIFEST)) fail()
...
function buildExpectedManifest() {
  const manifest = canonicalV085AcceptanceManifest()
  manifest.schemaVersion = V086_ACCEPTANCE_SCHEMA_VERSION
  manifest.package.channel = "unpublished"
  manifest.package.scope = "source-candidate"
  manifest.package.version = V086_PACKAGE_VERSION
  manifest.v082HistoricalRelease = {
    outcome: "published-latest-v082-release-is-immutable-and-not-reusable-for-this-unpublished-v086-source-candidate-or-any-later-package",
    reusableForV086: false,
    version: "0.8.2",
  }
  manifest.v083HistoricalRelease = {
    outcome: "published-0.8.3-release-is-immutable-and-not-reusable-for-this-unpublished-v086-source-candidate-or-any-later-package",
    reusableForV086: false,
    version: "0.8.3",
  }
  manifest.v084HistoricalRelease = {
    outcome: "published-0.8.4-release-is-immutable-and-not-reusable-for-this-unpublished-v086-source-candidate-or-any-later-package",
    reusableForV086: false,
    version: "0.8.4",
  }
  manifest.v085HistoricalRelease = {
    outcome: "immutable-v085-tag-release-is-unpublished-history-and-not-reusable-for-this-unpublished-v086-source-candidate-or-any-later-package",
    reusableForV086: false,
    version: "0.8.5",
  }
  ...
  manifest.authority.fixturePlan.registryInstall = "requires-authorized-release-before-registry-install-persona-harness@0.8.6"
  manifest.authority.hostedFixture.revision = "v086-source-candidate-head-before-authorized-release"
  ...
  manifest.hostedResidual.id = "v086-current-package-acceptance-and-authorized-current-artifact-observation"
...
function fail() {
  throw new V086AcceptanceManifestError("v086-acceptance-schema")
}
