...
const BETA28_PACKAGE_VERSION = "0.8.0-beta.28"
const ACCEPTANCE_PATH = join("docs", "current", "release", "consumer-authority-beta28-acceptance.json")
...
export function canonicalBeta28AcceptanceManifest() {
  return structuredClone(EXPECTED_MANIFEST)
}

export function readBeta28AcceptanceManifest(packageRoot) {
  let packageVersion
  let value
  try {
  ...
  return parseBeta28AcceptanceManifest(value, packageVersion)
}

export function parseBeta28AcceptanceManifest(value, packageVersion) {
  if (packageVersion !== BETA28_PACKAGE_VERSION || !isDeepStrictEqual(value, EXPECTED_MANIFEST)) fail()
...
function buildExpectedManifest() {
  ...
  manifest.schemaVersion = BETA28_ACCEPTANCE_SCHEMA_VERSION
  manifest.package.version = BETA28_PACKAGE_VERSION
  ...
  manifest.beta27HistoricalObserverTool = {
    outcome: "protected-verify-selector-collapsed-runner-package-list-record-assessment-private-copy-and-output-handoff-failures-to-one-non-attributable-code-not-reusable-as-closure-evidence",
    reusableForBeta28: false,
    version: "0.8.0-beta.27",
  }
  manifest.authority.fixturePlan.registryInstall = "npm install persona-harness@0.8.0-beta.28 --registry https://registry.npmjs.org"
  manifest.authority.hostedFixture.revision = "postmerge-persona-harness-beta28-main-sha"
  procedure.cleanliness = v4Cleanliness()
  procedure.observerGhTool = canonicalObserverGhToolContract()
  procedure.observerGhSelection = "workflow-owned-dpkg-record-qualified-private-copy-with-fixed-nonreflective-selector-stage-before-every-package-contract"
  ...
  procedure.prefetchSteps = [
    ...
    "workflow-selected-observer-gh-tool-dpkg-record-qualified-regular-nonsymlink-executable-version-compatible-fixed-selector-stage-and-no-package-path-lookup",
  ...
  manifest.closureCompleteness = {
    ...
    localProof: "clean-linux-node20-source-built-and-fresh-packed-installed-v4-cleanliness-qualified-dpkg-workflow-selected-gh-selector-lifecycle-parser-fixed-stage-diagnostics-and-authority-child-contracts",
  ...
  manifest.hostedResidual = {
    id: "beta28-v4-prearmed-same-commit-current-artifact-fetch-consume-replay",
...
function fail() {
  throw new Beta28AcceptanceManifestError("beta28-acceptance-schema")
}
