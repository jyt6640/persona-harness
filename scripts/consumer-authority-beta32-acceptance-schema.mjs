...
import { canonicalBeta31AcceptanceManifest } from "./consumer-authority-beta31-acceptance-schema.mjs"
...
const BETA32_PACKAGE_VERSION = "0.8.0-beta.32"
const ACCEPTANCE_PATH = join("docs", "current", "release", "consumer-authority-beta32-acceptance.json")
...
export function canonicalBeta32AcceptanceManifest() {
  return structuredClone(EXPECTED_MANIFEST)
}

export function readBeta32AcceptanceManifest(packageRoot) {
  let packageVersion
  let value
  try {
  ...
  return parseBeta32AcceptanceManifest(value, packageVersion)
}

export function parseBeta32AcceptanceManifest(value, packageVersion) {
  if (packageVersion !== BETA32_PACKAGE_VERSION || !isDeepStrictEqual(value, EXPECTED_MANIFEST)) fail()
...
function buildExpectedManifest() {
  const manifest = canonicalBeta31AcceptanceManifest()
  const procedure = manifest.prearmedExternalHandoff.finalObserverProcedure
  manifest.schemaVersion = BETA32_ACCEPTANCE_SCHEMA_VERSION
  manifest.package.version = BETA32_PACKAGE_VERSION
  manifest.beta31HistoricalPackageContract = {
    outcome: "protected-verify-revealed-stale-current-version-authority-and-incomplete-source-and-release-fixture-copy-graphs-before-the-real-source-and-fresh-tar-package-contracts-could-run",
    reusableForBeta32: false,
    version: "0.8.0-beta.31",
  }
  manifest.authority.fixturePlan.registryInstall = "npm install persona-harness@0.8.0-beta.32 --registry https://registry.npmjs.org"
  manifest.authority.hostedFixture.revision = "postmerge-persona-harness-beta32-main-sha"
  ...
  manifest.hostedResidual = {
    ...
    whyLocalCannotClose: "A real current signed artifact, leaf-certificate validity window, isolated external credential, online verification, and GitHub Actions discovery cannot be produced locally. Local source and packed contracts prove v4 tracked-binding, stage-scoped residue cleanliness, workflow-selected observer-gh ownership, runner-temp isolated token-free state, byte-strict primary package-record selector lifecycle with optional known completion mode-independent validation, strict secondary validation, fixed stage and shape diagnostics, current package-lock-acceptance binding, complete source and fresh-installed fixture import closure, Linux child-envelope behavior, public readiness, privacy, trusted modeled fetch, one Finish consumption, and replay block without accessing a live artifact.",
  }
  return manifest
}

function fail() {
  throw new Beta32AcceptanceManifestError("beta32-acceptance-schema")
}
