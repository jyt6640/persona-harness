...
const BETA16_PACKAGE_VERSION = "0.8.0-beta.16"
const ACCEPTANCE_PATH = join("docs", "current", "release", "consumer-authority-beta16-acceptance.json")
const EXPECTED_MANIFEST = {
  schemaVersion: BETA16_ACCEPTANCE_SCHEMA_VERSION,
  package: { channel: "staging", scope: "staging-only", version: BETA16_PACKAGE_VERSION },
  packageBoundary: {
  ...
  beta15HistoricalExternal: { outcome: "external-gh-attestation-observer-combined-mutually-exclusive-identity-selectors-before-crypto", reusableForBeta16: false, version: "0.8.0-beta.15" },
  ...
  authority: {
    ...
    fixturePlan: { artifact: "project-finish-attestation", consumer: "public-java-spring-gradle", postmergeAction: "normal-push-to-main", registryInstall: "npm install persona-harness@0.8.0-beta.16 --registry https://registry.npmjs.org" },
    hostedFixture: { callerWorkflowPath: ".github/workflows/research-attestation.yml", certificateSanIdentity: "reusable-producer-workflow", event: "push", ref: "refs/heads/main", repository: "jyt6640/persona-harness-attestation-claim-fixture", reusableWorkflowPath: ".github/workflows/persona-harness-project-finish.yml", revision: "postmerge-persona-harness-beta16-main-sha" },
  ...
  externalAttestationCommandPlan: canonicalExternalAttestationCommandPlan(),
  prearmedExternalHandoff: {
    prepare: {
      allowedBeforeFixture: ["prepare-isolated-consumer-home", "enroll", "status", "explain", "observer-credential-preflight", "external-attestation-command-plan-preflight", "public-pre-authority-readiness", "public-initialized-finish-state"],
      ...
      attestationCommandPreflight: { command: "node node_modules/persona-harness/scripts/preflight-consumer-authority-external-attestation.mjs --json", credential: "absent", input: "no-artifact-grammar-sentinel-only", output: "bounded-exit-classification-only", productFallback: "forbidden" },
    ...
    trigger: { onlyAfter: "observer-credential-preflight-ready-attestation-command-plan-preflight-ready-public-initialized-readiness-and-natural-current-version-original-artifact", steps: ["observer-credential-preflight-ready", "external-attestation-command-plan-parser-ready", "public-initialized-finish-blocked-only-on-trusted-authority-required", "download-original-bytes-for-independent-online-verification", "verify-online-before-leaf-certificate-notAfter", "authority-fetch-discovers-and-binds-original-artifact", "finish-consume-once", "finish-replay-blocked"] },
    nonAuthority: ["preflight-does-not-self-validate", "readiness-does-not-authorize-fixture", "readiness-does-not-grant-authority", "readiness-does-not-persist-or-log-host-credential", "command-plan-preflight-does-not-access-artifact-or-network", "command-plan-preflight-does-not-reuse-beta15-artifact"],
  },
  hostedResidual: { id: "beta16-prearmed-external-authority-consumption", requiredEvidence: "a bounded observer credential-preflight ready result, a no-token no-artifact gh attestation command-plan parser-ready result, and a public initialized Finish blocked only on trusted-authority-required in the exact consumer before fixture authorization, one natural current-version public fixture artifact, independent online verification inside the live certificate window, real installed authority fetch, one explicit Finish consumption, and immediate replay rejection", whyLocalCannotClose: "GitHub Actions must mint the original current-version signed artifact, while independent online verification and the installed authenticated discovery route must observe live hosted state without retaining a credential." },
...
export function readBeta16AcceptanceManifest(packageRoot) {
  ...
  return parseBeta16AcceptanceManifest(value, packageVersion)
}

export function parseBeta16AcceptanceManifest(value, packageVersion) {
  if (packageVersion !== BETA16_PACKAGE_VERSION || !isDeepStrictEqual(value, EXPECTED_MANIFEST)) fail()
  parseExternalAttestationCommandPlan(value.externalAttestationCommandPlan)
  return value
}

function fail() {
  throw new Beta16AcceptanceManifestError("beta16-acceptance-schema")
}
