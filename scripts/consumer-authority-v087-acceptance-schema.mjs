...
import { canonicalV086AcceptanceManifest } from "./consumer-authority-v086-acceptance-schema.mjs"
...
const V087_PACKAGE_VERSION = "0.8.7"
const ACCEPTANCE_PATH = join("docs", "current", "release", "consumer-authority-v087-acceptance.json")
...
export function canonicalV087AcceptanceManifest() {
  return structuredClone(EXPECTED_MANIFEST)
}

export function readV087AcceptanceManifest(packageRoot) {
  let packageVersion
  let value
  try {
  ...
  return parseV087AcceptanceManifest(value, packageVersion)
}

export function parseV087AcceptanceManifest(value, packageVersion) {
  if (packageVersion !== V087_PACKAGE_VERSION || !isDeepStrictEqual(value, EXPECTED_MANIFEST)) fail()
...
function buildExpectedManifest() {
  const manifest = canonicalV086AcceptanceManifest()
  manifest.schemaVersion = V087_ACCEPTANCE_SCHEMA_VERSION
  manifest.package.channel = "unpublished"
  manifest.package.scope = "source-candidate"
  manifest.package.version = V087_PACKAGE_VERSION
  manifest.v082HistoricalRelease = {
    outcome: "published-latest-v082-release-is-immutable-and-not-reusable-for-this-unpublished-v087-source-candidate-or-any-later-package",
    reusableForV087: false,
    version: "0.8.2",
  }
  manifest.v083HistoricalRelease = {
    outcome: "published-0.8.3-release-is-immutable-and-not-reusable-for-this-unpublished-v087-source-candidate-or-any-later-package",
    reusableForV087: false,
    version: "0.8.3",
  }
  manifest.v084HistoricalRelease = {
    outcome: "published-0.8.4-release-is-immutable-and-not-reusable-for-this-unpublished-v087-source-candidate-or-any-later-package",
    reusableForV087: false,
    version: "0.8.4",
  }
  manifest.v085HistoricalRelease = {
    outcome: "immutable-v085-tag-release-is-unpublished-history-and-not-reusable-for-this-unpublished-v087-source-candidate-or-any-later-package",
    reusableForV087: false,
    version: "0.8.5",
  }
  manifest.v086HistoricalRelease = {
    outcome: "published-latest-v086-release-is-immutable-and-not-reusable-for-this-unpublished-v087-source-candidate-or-any-later-package",
    reusableForV087: false,
    version: "0.8.6",
  }
  ...
  manifest.authority.fixturePlan.registryInstall = "requires-authorized-release-before-registry-install-persona-harness@0.8.7"
  manifest.authority.hostedFixture.revision = "v087-source-candidate-head-before-authorized-release"
  ...
  manifest.hostedResidual.id = "v087-current-package-acceptance-and-authorized-current-artifact-observation"
...
function fail() {
  throw new V087AcceptanceManifestError("v087-acceptance-schema")
}
