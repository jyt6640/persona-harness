...
const V084_PACKAGE_VERSION = "0.8.4"
const ACCEPTANCE_PATH = join("docs", "current", "release", "consumer-authority-v084-acceptance.json")
...
export function canonicalV084AcceptanceManifest() {
  return structuredClone(EXPECTED_MANIFEST)
}

export function readV084AcceptanceManifest(packageRoot) {
  let packageVersion
  let value
  try {
  ...
  return parseV084AcceptanceManifest(value, packageVersion)
}

export function parseV084AcceptanceManifest(value, packageVersion) {
  if (packageVersion !== V084_PACKAGE_VERSION || !isDeepStrictEqual(value, EXPECTED_MANIFEST)) fail()
...
function buildExpectedManifest() {
  ...
  manifest.schemaVersion = V084_ACCEPTANCE_SCHEMA_VERSION
  manifest.package.channel = "unpublished"
  manifest.package.scope = "source-candidate"
  manifest.package.version = V084_PACKAGE_VERSION
  manifest.v082HistoricalRelease = {
    outcome: "published-latest-v082-release-is-immutable-and-not-reusable-for-this-unpublished-v084-source-candidate-or-any-later-package",
    reusableForV084: false,
    version: "0.8.2",
  }
  manifest.v083HistoricalRelease = {
    outcome: "published-0.8.3-release-is-immutable-and-not-reusable-for-this-unpublished-v084-source-candidate-or-any-later-package",
    reusableForV084: false,
    version: "0.8.3",
  }
  ...
  manifest.authority.fixturePlan.registryInstall = "requires-authorized-release-before-registry-install-persona-harness@0.8.4"
  manifest.authority.hostedFixture.revision = "v084-source-candidate-head-before-authorized-release"
  ...
  manifest.hostedResidual.id = "v084-current-package-acceptance-and-authorized-current-artifact-observation"
...
function fail() {
  throw new V084AcceptanceManifestError("v084-acceptance-schema")
}
