export function createPlanOnlyPrompt(): string {
  return [
    ...
    "",
    "만약 사용자가 '플랜 보고 구현해줘', '계획대로 해줘', 'README 보고 구현해줘', '이제 구현해줘'처럼 짧은 구현 지시를 하면 바로 구현하지 말고 `npx ph workflow check`와 `npx ph plan --implement`를 먼저 실행해줘.",
    "",
...
export function createImplementationPrompt(): string {
  return [
    "구현을 시작하기 전에 `npx ph workflow check`와 `npx ph plan --implement`를 먼저 실행하고, README.md, .persona/project-profile.jsonc, .persona/policies, .persona/workflow/plan.md를 읽은 뒤 accepted plan 기준으로 구현해줘.",
    "",
