...
export function passedImplementOutput(projectDir: string, options: { readonly full?: boolean } = {}): CliRunResult {
  const uniqueLines = [
  ...
  const railBodyLines = [
    ...
    "2. After Java files are generated or changed, list them so you know what to read next.",
    ...
    "3. Record a read of each Java file with `npx ph evidence read <path>`, and of `.persona/project-profile.jsonc`.",
    "   Listing files does not record a read: `bearshell` stores only a hash of its output, so `java-role-read-coverage` is satisfied by `ph evidence read` alone.",
    "   If Persona Harness adds a Java Role Read Follow-up, record a read of those representative Java files too before continuing.",
    ...
    "11. Run `npx ph workflow finish implement`. Without external attestation this reports what is still missing and exits nonzero; that is the gate working, not a failure to fix.",
    "12. For a cooperative PASS, Persona Harness runs the build itself, so it needs a git worktree root and a build that actually executes:",
    "    - `git init` and commit if the project is not already a git worktree.",
    "    - `./gradlew clean` first — an UP-TO-DATE build is reported as `build-task-nonfresh`.",
    "    - `npx ph workflow finish implement --assurance cooperative`.",
    "13. Do not give the final answer until finish passes.",
    "",
    "Scope:",
    "- AI-facing workflow rail",
  ...
  return {
    status: 0,
    stdout: cachedWorkflowRailOutput({
      full: options.full ?? false,
      projectDir,
      railBodyLines,
      surface: "workflow-implement",
      uniqueLines,
    }),
    stderr: "",
  }
