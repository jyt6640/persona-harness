...
export function readWorkflowFinishAuthority(
  projectDir: string,
  options: { readonly consumeExternalAttestation?: boolean } = {},
  ...
  const externalAttestation = verifyExternalFinishAttestation(projectDir, new Date(), { consume: options.consumeExternalAttestation })
