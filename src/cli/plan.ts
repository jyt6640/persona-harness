...
function createImplementationReportTemplate(projectDir: string): string {
  return [
    ...
    "",
    "- [ ] README.md를 끝까지 읽었다.",
    "- [ ] .persona/workflow/plan.md를 끝까지 읽었다.",
    "- [ ] 긴 파일은 `npx ph bearshell --shell 'sed -n \"1,220p\" <file>'` 형식으로 범위를 나눠 읽었다.",
    "- [ ] 읽지 못한 범위가 있으면 아래 Continuation에 남겼다.",
    "",
    "## Implemented Files",
    "",
...
function createReviewReportTemplate(projectDir: string): string {
  return [
    ...
    "## Requirements Check",
    "",
    "- [ ] README/plan read coverage가 implementation report에 남아 있다.",
