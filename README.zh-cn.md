...
> 稳定 npm 通道为 `persona-harness@latest=0.5.0`。release prep 目标是预发布 `next` 通道的 `0.6.0-rc.1`。在 QA publish GO 和 registry gitHead/shasum 验证之前，registry `next` 仍为 `0.5.0-rc.2`，alpha 仍为 `0.3.9-alpha.8`。
