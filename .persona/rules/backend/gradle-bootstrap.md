...
- Default guidance: `build.gradle` 또는 `build.gradle.kts`는 Spring Boot backend를 만들기 위한 최소 plugin/dependency/source compatibility만 다루며, 호환되는 Spring Boot plugin / Gradle launcher or wrapper / JDK toolchain 조합을 먼저 선택하고 그 선택을 build file에 명시한다. system Gradle이 없으면 wrapper를 우선 생성/사용하며, 가능하면 Gradle wrapper(`gradlew`, `gradlew.bat`, `gradle/wrapper/`)를 포함해 macOS/Linux `./gradlew test`, `./gradlew build`, `./gradlew bootRun`, Windows에서는 `./gradlew.bat test` 또는 `gradlew.bat test`로 검증할 수 있게 한다. JUnit Platform 실행에는 `testRuntimeOnly "org.junit.platform:junit-platform-launcher"` 또는 동등한 runtime launcher를 포함한다.
