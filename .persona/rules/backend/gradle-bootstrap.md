...
- Gradle build file에서는 Java/Spring Boot backend 기준을 유지하고 Maven `pom.xml`을 생성하지 않는다. Hard rule: `gradle test`, `gradle build`, `bootRun` 결과를 Node/JS/Python/shell shim이나 `tools/gradle-shim.js` 같은 fake Gradle script로 흉내 내지 않는다. 실제 Gradle 실행이 불가능하면 성공처럼 말하지 말고 Gradle 실행 환경 문제로 보고한다.
- `settings.gradle` 또는 `settings.gradle.kts`는 하나의 Spring Boot application root project 이름을 명확히 둔다.
- Default guidance: `build.gradle` 또는 `build.gradle.kts`는 Spring Boot backend를 만들기 위한 최소 plugin/dependency/source compatibility만 다루며, 호환되는 Spring Boot plugin / Gradle launcher or wrapper / JDK toolchain 조합을 먼저 선택하고 그 선택을 build file에 명시한다. 가능하면 Gradle wrapper(`gradlew`, `gradlew.bat`, `gradle/wrapper/`)를 포함해 `./gradlew test`, `./gradlew build`, `./gradlew bootRun`으로 검증할 수 있게 하며, JUnit Platform 실행에는 `testRuntimeOnly "org.junit.platform:junit-platform-launcher"` 또는 동등한 runtime launcher를 포함한다.
